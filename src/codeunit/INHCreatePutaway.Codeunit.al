codeunit 50175 INHCreatePutaway
{
    // +---------------------------------------------+
    // +                                             +
    // +           Inhaus Handels GmbH               +
    ...
    // ------------------------------------------------------------
    // C54°          RBI  12.04.19  Wenn der Lagerplatz im Einlagerungsvorschlag gefüllt ist, diesen in die Einlagerungen übernehmen
    ...
    begin
        ...
        with PostedWhseRcptLine do begin
            ...
            LineNo := LineNo + 10000;
            //START C54° ---------------------------------
            if ("To Zone Code" <> '') and ("To Bin Code" <> '') then begin
                Clear(Bin);
    ...
    local procedure fnk_MakeCrossDock(): Boolean
    var
        lo_re_Location: Record Location;
        lo_re_RegisteredWhseActivityLine: Record "Registered Whse. Activity Line";
    begin
        // *** Die Registrierten Einlagerungen prüfen ob für die Position bereits das Cross Docking durchgeführt wurde
        // *** Da beim Buchen des WE immer automatisch die Einlagerung mit der gesamten Cross Docking Menge registriert wird
        // *** muss nicht auf die Menge geprüft werden
        if lo_re_Location.Get(PostedWhseRcptLine."Location Code") and (lo_re_Location."Cross-Dock Bin Code" <> '') then begin
            lo_re_RegisteredWhseActivityLine.SetRange("Activity Type", lo_re_RegisteredWhseActivityLine."Activity Type"::"Put-away");
            lo_re_RegisteredWhseActivityLine.SetRange("Whse. Document Type", lo_re_RegisteredWhseActivityLine."Whse. Document Type"::Receipt);
            lo_re_RegisteredWhseActivityLine.SetRange("Whse. Document No.", PostedWhseRcptLine."No.");
            lo_re_RegisteredWhseActivityLine.SetRange("Whse. Document Line No.", PostedWhseRcptLine."Line No.");
            lo_re_RegisteredWhseActivityLine.SetRange("Bin Code", lo_re_Location."Cross-Dock Bin Code");
            lo_re_RegisteredWhseActivityLine.SetFilter(Quantity, '<>%1', 0);
            if lo_re_RegisteredWhseActivityLine.IsEmpty then
                exit(true);
        end;
    end;
