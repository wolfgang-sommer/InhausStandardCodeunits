codeunit 50126 INHPurchPost
{
    ...
    // ------------------------------------------------------------
    // Axx°          RBI  08.08.08  Einkaufsverwaltung
    // Axx°.1        SSC  28.11.13  "Zugewiesene Benutzer-ID" vom Logistikbeleg in Posten übernehmen
    // Axx°.2        SSC  23.01.19  Kred. RE/GS Nr. nur 3 Jahre zurück prüfen, da manche Kreditoren Nummernkreise neu verwenden (siehe auch Cu12,Axx°.2 und Cu50033,Axx°.8)
    // A04°.2        SSC  20.04.16  Möglichkeit Verfügbarkeitsprüfung auszulassen
    // B05°          RBI  02.06.10  Bei Direktlieferung prüfen und Mwst.- und Prosuktbuchungsgruppe umstellen.
    // B29°          SSC  15.05.18  IF Guiallowed vor Confirm
    // B40°.1        SSC  14.04.22  Keine Archivierung beim Buchen
    // C54°          RBI  12.11.19  Funktion ShouldPostWhseJnlLine - Bei Lagerorten mit LVS und beim direkten Buchen über EK-Reklamation einen Lagerplatzposten schreiben
    //                              Funktion CheckWarehouse - Prüfung bei direktem Buchen umgehen
    // 
    // Beachte: Clearall in OnRun: Funktionen um globale Variablen von außen zu setzen funktionieren mit OnRun nicht
    // 
    // //foobaz:
    //  - foobazCu90 in Cu50033
    ...
    local procedure GetPostingDate(PurchaseHeader: Record "Purchase Header"): Date
    begin
    ...
    begin
        ...
        with PurchLine do begin
            ...
            if PurchHeader."Currency Code" <> '' then begin
                Usedate := GetPostingDate(PurchHeader);
            ...
            if OldPurchLine.FindSet then
                repeat
                    ...
                    if InsertPurchLine then begin
                        ...
                        //START Axx° ---------------------------------
                        //        NewPurchLine.INSERT;
                        if not NewPurchLine.Insert then
                            NewPurchLine.Modify;
                        //STOP Axx° ----------------------------------
                    end;
                    if RoundingLineInserted then
    ...
    var
        ...
        IsHandled: Boolean;
    begin
        exit;   //Beim buchen soll nicht archiviert werden, zu viele Archive durch alternative Einlagerung   //B40°.1
        IsHandled := false;
    ...
    begin
        ...
        if not GuiAllowed then exit;   //B29°
        with PurchHeader do begin
            if not Invoice then
                exit;
    ...
    begin
        ...
        VendLedgEntry.SetFilter("Posting Date", '>%1', CalcDate('<-3Y>'));   //Axx°.2
        if VendLedgEntry.FindFirst then
            Error(
              PurchaseAlreadyExistsErr, VendLedgEntry."Document Type", GenJnlLineExtDocNo);
    end;

    local procedure UpdateVATAmountsOnReverseChargeInvoicePostingBuffer(PurchHeader: Record "Purchase Header"; var TempInvoicePostBuffer: Record "Invoice Post. Buffer" temporary)
    var
                    ...
                    case "VAT Calculation Type" of
                        "VAT Calculation Type"::"Reverse Charge VAT":
                            begin
                                ...
                                VATBaseAmount := "VAT Base Amount" * (1 - PurchHeader."VAT Base Discount %" / 100);
                                VATBaseAmountACY := "VAT Base Amount (ACY)" * (1 - PurchHeader."VAT Base Discount %" / 100);

                                if PurchHeader."Currency Code" <> '' then
                                    VATBaseAmount := CurrExchRate.ExchangeAmtLCYToFCY(
                                        GetPostingDate(PurchHeader), PurchHeader."Currency Code",
                                ...
                                if TempInvoicePostBufferReverseCharge.Find then begin
                                    ...
                                    "VAT Amount" := Round(VATAmountRemainder, CurrencyDocument."Amount Rounding Precision");
                                    VATAmountRemainder -= "VAT Amount";

                                    if PurchHeader."Currency Code" <> '' then
                                        "VAT Amount" := Round(CurrExchRate.ExchangeAmtFCYToLCY(
                                              GetPostingDate(PurchHeader), PurchHeader."Currency Code",
                                              "VAT Amount", PurchHeader."Currency Factor"));

                                    VATAmountACYRemainder += VATAmountACY;
                                    "VAT Amount (ACY)" := Round(VATAmountACYRemainder, Currency."Amount Rounding Precision");
                                    VATAmountACYRemainder -= "VAT Amount (ACY)";

                                    "VAT Base Amount" := Round("VAT Base Amount" * (1 - PurchHeader."VAT Base Discount %" / 100));
                                    "VAT Base Amount (ACY)" := Round("VAT Base Amount (ACY)" * (1 - PurchHeader."VAT Base Discount %" / 100));
                                end else begin
                                    if PurchHeader."Currency Code" <> '' then
                                        VATAmount := Round(
                                            CurrExchRate.ExchangeAmtFCYToLCY(GetPostingDate(PurchHeader),
                                              PurchHeader."Currency Code", VATAmount, PurchHeader."Currency Factor"))
                                    else
                                        VATAmount := Round(VATAmount);

                                    "VAT Amount" := VATAmount;
                                    "VAT Amount (ACY)" := Round(VATAmountACY, Currency."Amount Rounding Precision");
    ...
    begin
        ...
        UpdateVATAmountsOnReverseChargeInvoicePostingBuffer(PurchHeader, TempInvoicePostBuffer);
    ...
    local procedure OnAfterPostItemLine(PurchaseLine: Record "Purchase Line"; CommitIsSupressed: Boolean)
    begin
    end;
    ...
    [IntegrationEvent(false, false)]
    local procedure OnBeforePostVendorEntry(var GenJnlLine: Record "Gen. Journal Line"; var PurchHeader: Record "Purchase Header"; var TotalPurchLine: Record "Purchase Line"; var TotalPurchLineLCY: Record "Purchase Line"; PreviewMode: Boolean; CommitIsSupressed: Boolean)
    begin
    end;
    ...
    [IntegrationEvent(false, false)]
    local procedure OnBeforePostUpdateOrderLine(PurchHeader: Record "Purchase Header"; var TempPurchLineGlobal: Record "Purchase Line" temporary)
    begin
    end;
    ...
    [Scope('Internal')]
    procedure fnk_GetWhseRcptHeader(var par_re_WhseRcptHeader: Record "Warehouse Receipt Header")
    begin
        //A04°.2
        par_re_WhseRcptHeader := WhseRcptHeader;
    end;
}
