codeunit 50178 INHCopyDocumentMgt
{
    // -----------------------------------------------------
    ...
    // 
    // 
    // !!! Achtung: Bei Anpassungen auch immer die entsprechenden Archiv Funktionen beachten !!!
    // 
    // +---------------------------------------------+
    // +                                             +
    // +           Inhaus Handels GmbH               +
    ...
    // ------------------------------------------------------------
    // Axx°          JLE  31.07.08  Codeunit übernommen/gemergt
    //               RBI  05.02.09  Attached Items mitkopieren
    //               RBI  05.02.09  Die ExtendedTexts nicht zusätzlich einfügen, aber die Attached to Line No. mit übernehmen
    //               MBA  27.02.09  Buchungsdatum & Belegdatum soll trotz "inkl. Kopf" kopieren immer das aktuelle Datum sein
    //               RBI  09.04.09  Wenn die "Line No." = "Attached to Line No." ist, dann ExtendedText nicht einfügen
    //               SSC  06.05.10  Beim Kopieren in eine Rekla./Gutschrift soll Projektnr. mitgezogen werden
    //               SSC  24.08.11  Zugesagtes- und Warenausgangsdatum in den Zeilen müssen gleich sein wie im Ursprungsbeleg
    //               SSC  09.06.20  Auftragsart nicht kopieren wenn die Belegarten unterschiedlich sind
    //               SSC  29.09.22  Gesperrte Artikel dürfen auch kopiert werden
    // Axx°.2        SSC  02.05.12  Beim kopieren von gebuchter VK-Rechnung Möglichkeit geben auf Auftragsnummer zu filtern
    //               SSC  20.09.12  Info-Zeilen zum Auftrag erstellen
    //               SSC  18.04.14  Filter auf Artikel ermöglichen
    //               SSC  14.06.21  Zusätzlich zum Auftrag auch Lieferschein filtern ermöglichen (Erweiterung in fnk_OnBeforeCopySalesInvLinesToDoc)
    //               SSC  04.10.21  Artikeltyp::Setkomponente-Flag darf nicht kopiert werden wenn der Set dazu nicht mitkopiert wurde
    // Axx°.4        SSC  15.01.14  Lagerort vorbelegen
    // Axx°.6        SSC  16.05.14  VK-Beleg nach dem Kopieren nie freigeben
    // Axx°.7        SSC  07.04.15  "Gehört zu Zeilennr." richtig kopieren
    //               SSC  13.10.16  Ergänzt in CopySalesCrMemoLinesToDoc; auch für "Textzeilen" mit Typ=Artikel
    // Axx°.9        SSC  24.02.16  Div Felder müssen immer vom Kopf übernommen werden, auch wenn "Zeilen neu berechnen"=Nein, Kreditor
    //               SSC  07.03.16  Projektnr.
    // Axx°.10       SSC  09.02.17  Automatischer Prozess mitgeben
    // Axx°.11       SSC  12.07.18  Beim kopieren vom DIV auch den Lagerortcode mitnehmen
    // Axx°.12       SSC  18.03.19  Beim kopieren in eine VK-Reklamation das Auftragsdatum alt lassen, wegen der Preisfindung
    // Axx°.13       SSC  25.07.19  In der Zeile müssen div. Felder immer neu berechnet werden, auch wenn "Zeilen neu berechnen"=Nein, Artikelart
    // Axx°.14       SSC  29.07.19  Meldung wenn bei einer Retoure die Menge einen Wert überschreitet
    // Axx°.15       SSC  28.09.21  Meldung "Es wurde Skonto gewährt" nicht bringen beim kopieren in VK-Beleg
    // Axx°.16       SSC  15.02.22  Berücksichtigen wenn es Textbausteincodes nicht mehr gibt
    // A65°          MBA  15.07.09  GU-Angebot
    // A97°.1        SSC  01.12.17  Buchungsgruppen auch bei Div übernehmen
    // B12°          SSC  25.08.10  Nur Super User dürfen Anzahlungs-Sachkonten Zeilen kopieren
    // B17°          SSC  28.08.13  Gruppenpreis
    // B28°          SSC  13.10.17  Bonusgutschrift
    // B43°.1        SSC  13.07.18  Default keine Kreditlimitprüfung bei Beleg kopieren
    // B44°          SSC  25.10.11  Variantenartikel
    //               SSC  07.02.13  Erweiterungen für Einkauf
    // B74°          SSC  15.05.13  Montagegruppe immer kopieren
    // B75°          SSC  15.05.13  Liefertermingruppen immer kopieren
    // B85°          SSC  07.11.13  §19/Bauleistung DE/... auf Zeilenbasis
    // B99°          SSC  20.04.17  Zolldaten kopieren bei DIV, wenn Änderung der From-Zeile nicht zu lange her ist
    // C01°          SSC  11.01.16  Beim kopieren vom DIV die EP-Felder übernehmen
    // C03°          SSC  02.12.14  Reservierung
    // C11°          SSC  23.03.15  Webshoprabatt darf nicht kopiert werden (Ausnahme: Retoure/Gutschrift)
    // C24°          SSC  07.11.16  Bug: Chargenart. werden nicht kopiert von Geb. GS wenn EP-Rückverfolgung(Debitoren&Verkauf Einr.)=Ja
    // C27°          SSC  13.09.17  Hooks
    // C30°          SSC  14.07.16  Buchungsmatrix Neustrukturierung
    // C47°          SSC  24.09.18  Neues Feld Kreditorenartikelnr. im VK
    // C54°          RBI  11.12.19  fnk_OnBeforeInsertSalesLine - Bei Verkaufsreklamation Standard-Lagerplatz füllen
    //                              fnk_OnBeforeInsertPurchLine - Bei Einkaufsreklamation Standard-Lagerplatz füllen
    //               SSC  09.04.20  Doch nicht machen, wieder auskommentiert
    // C61°          SSC  31.03.20  Sortimentseinteilung immer neu berechnen
    // C73°          SSC  23.02.21  Nettoprojekt immer nein, da in Zukunft nicht mehr in Verwendung
    //               SSC  04.06.24  Wieder aktiv
    // C83°          SSC  27.02.24  SST
    // C92°          SSC  16.01.24  Mwst. Buchungsgruppen nicht übernehmen wenn nicht mehr gültig, auch bei nicht neuberechnen
    // C95°          SSC  30.06.25  Gruppierung
    // 
    // !!! Achtung: Bei Anpassungen auch immer die entsprechenden Archiv Funktionen beachten !!!
    ...
    var
        ...
        "+++TE_INHAUS+++": ;
        TextKom: Label 'Kommission: ';
        TextOrderNo: Label 'Auftragsnr.: %1 ';
        TextShipNo: Label 'Lieferungsnr.: %1';
    ...
    begin
        with ToSalesHeader do begin
            ...
            cu_DialogMgt.fnk_OpenDialog2(1, 'Kopiere Zeile', 0);   //Axx°

            if IncludeHeader then begin
                lo_re_OldSalesHeader := ToSalesHeader;   //C27°
    ...
    local procedure CopySalesDocInvLine(FromSalesInvHeader: Record "Sales Invoice Header"; var ToSalesHeader: Record "Sales Header"; var LinesNotCopied: Integer; var MissingExCostRevLink: Boolean)
    var
        FromSalesInvLine: Record "Sales Invoice Line";
    begin
        //Axx°.2: Parameter ToSalesHeader auf VAR=True umgestellt, weil Änderungen in fnk_OnBeforeCopySalesInvLinesToDoc passieren
        with ToSalesHeader do begin
            FromSalesInvLine.Reset;
            FromSalesInvLine.SetRange("Document No.", FromSalesInvHeader."No.");
            if MoveNegLines then
                FromSalesInvLine.SetFilter(Quantity, '<=0');
            fnk_OnBeforeCopySalesInvLinesToDoc(ToSalesHeader, FromSalesInvLine);   //Axx°.2   TODO: ins Event auslagern, (wenn der Rest auch möglich)
    ...
    begin
        with ToSalesHeader do begin
            ...
            if "Document Type" in ["Document Type"::"Return Order"] then begin
                ToSalesHeader."Document Date" := WorkDate;
                ToSalesHeader."Posting Date" := WorkDate;
            end;
            if (FromDocType in [SalesDocType::"Posted Invoice", SalesDocType::"Posted Shipment", SalesDocType::"Posted Return Receipt", SalesDocType::"Posted Credit Memo"
                                , SalesDocType::"Arch. Quote", SalesDocType::"Arch. Order", SalesDocType::"Arch. Blanket Order", SalesDocType::"Arch. Return Order"])
               or ("Document Type" in ["Document Type"::"Return Order"])
            then begin
                ToSalesHeader.Validate("Posting Date", WorkDate);
            end;
            if "Document Type" in ["Document Type"::"Blanket Order", "Document Type"::Quote, "Document Type"::Order] then begin
                lo_re_Cust.Get("Bill-to Customer No.");
                if ("Payment Terms Code" <> lo_re_Cust."Payment Terms Code") then begin
                    Validate("Payment Terms Code", lo_re_Cust."Payment Terms Code");
                end;
            end;
            case FromDocType of
                SalesDocType::"Posted Shipment":
                    ...
                    begin
                        if "Document Type" = "Document Type"::"Return Order" then begin
                            ...
                            if lo_bo_JustOneShptInInv then begin
                                ...
                                if lo_re_FromSalesInvLine.FindFirst then begin
                                    "Your Reference" := lo_re_FromSalesInvLine."Description 2";
                                end;
                                lo_re_FromSalesInvLine.SetFilter(Description, '%1', 'Ihre Bestellnummer:*');
                                if lo_re_FromSalesInvLine.FindFirst then begin
                                    "Ihre Bestellnummer" := lo_re_FromSalesInvLine."Description 2";
                                end;
                            end;
                        end;
                    end;
            ...
            //START C03° ---------------------------------
            if lo_re_Cust.Get("Sell-to Customer No.") then;
            ...
            //STOP  A65° ---------------------------------
            lo_re_ShptDateGrpDoc.fnk_CopySalesHdr(FromSalesHeader, ToSalesHeader);   //B75°
    ...
    begin
        ...
        if ReleaseDocument then begin
            ...
            if (FromDocType in
               ...
               and not IncludeHeader and not RecalculateLines
            then
                if FromPurchHeader.Status = FromPurchHeader.Status::Released
                then begin
    ...
    begin
        // !!! Achtung: bei Änderungen auch CopyArchSalesLine + Cu50023.fnk_CopySalesLines beachten   //Axx°
        CopyThisLine := true;
        ...
        if ((ToSalesHeader."Language Code" <> FromSalesHeader."Language Code") or RecalculateLines) and
           //Axx°:(FromSalesLine."Attached to Line No." <> 0) OR
        ...
        //START Axx° ---------------------------------
        if fnk_OnBeforeUpdateSalesLine(ToSalesHeader, ToSalesLine, FromSalesHeader, FromSalesLine, CopyThisLine) then begin
            exit;
        end;
        //STOP  Axx° ---------------------------------
        ...
        fnk_OnAfterUpdateSalesLine(ToSalesHeader, ToSalesLine, FromSalesHeader, FromSalesLine, CopyThisLine);   //Axx°
        ...
        fnk_OnBeforeInsertSalesLine(ToSalesHeader, ToSalesLine, FromSalesHeader, FromSalesLine, CopyThisLine);   //Axx°
    ...
    var
        ...
        InvDiscountAmount: Decimal;
    begin
        // !!! Achtung: bei Änderungen auch CopyArchPurchLine beachten   //Axx°
        CopyThisLine := true;
        ...
        fnk_OnBeforeInsertPurchLine(ToPurchHeader, ToPurchLine, FromPurchHeader, FromPurchLine, CopyThisLine);   //Axx°
    ...
    var
        ...
        DeferralDocType: Integer;
    begin
        fnk_ClearPurchICFields(ToPurchLine);   //Axx°
        ...
        if RecalculateLines and not FromPurchLine."System-Created Entry" then begin
            ...
            ToPurchLine."Buy-from Vendor No." := ToPurchHeader."Buy-from Vendor No.";   //Axx°.9
        end;
    ...
    var
        ...
        ItemChargeAssgntNextLineNo: Integer;
        "+++LO_VAR_INHAUS+++": Boolean;
        lo_co_LastShptNo: Code[20];
        lo_co_LastOrderNo: Code[20];
    begin
        MissingExCostRevLink := false;
        InitCurrency(ToSalesHeader."Currency Code");
        ...
        with FromSalesInvLine do
            if FindSet then
                repeat
                    //START Axx°.2 ---------------------------------
                    if bo_WriteOrderInfoLines then begin
                        if (lo_co_LastShptNo <> IC_Lieferschein) or (lo_co_LastOrderNo <> Auftragsnummer) then begin
                            fnk_WriteOrderInfoLines(ToSalesHeader, FromSalesInvLine, TempSalesLineBuf, NextLineNo);
                        end;
                    end;
                    //STOP  Axx°.2 ---------------------------------
                    FromLineCounter := FromLineCounter + 1;
                    ...
                    //START Axx°.2 ---------------------------------
                    lo_co_LastOrderNo := Auftragsnummer;
                    lo_co_LastShptNo := IC_Lieferschein;
                    //STOP  Axx°.2 ---------------------------------
                    //START Axx°.7 ---------------------------------
                    re_RealFromLineNoBuffer."Old Line Number" := FromSalesInvLine."Line No.";
                    re_RealFromLineNoBuffer."New Line Number" := TempSalesLineBuf."Line No.";
            ...
            if FindSet then begin
                ...
                repeat
                    ...
                    then begin
                        ...
                        if FromSalesInvLine.Get("Shipment No.", "Return Receipt Line No.") then begin
                            //STOP  Axx°.2 ---------------------------------
            ...
            if FindSet then begin
                ...
                repeat
                    ...
                    if ("Document No." <> OldReturnRcptDocNo) and ("Shipment Line No." > 0) then begin
                        ...
                        //C30°:InsertOldSalesCombDocNoLine(ToSalesHeader,OldCrMemoDocNo,OldReturnRcptDocNo,NextLineNo,FALSE);
                    end;
    ...
    begin
        ...
        ToSalesLine2.Type := ToSalesLine2.Type::" ";   //Axx°.2
    ...
    var
        ...
        CopyThisLine: Boolean;
    begin
        // !!! Achtung: bei Änderungen auch CopySalesLine + Cu50023.fnk_CopySalesLines beachten
        CopyThisLine := true;
        ...
        fnk_ClearSalesICFields(ToSalesLine);   //Axx°
        CopyThisLine := fnk_CheckIfCopyLine(ToSalesLine, CopyThisLine);   //B12°
        ...
        fnk_OnAfterUpdateArchSalesLine(ToSalesHeader, ToSalesLine, FromSalesHeaderArchive, FromSalesLineArchive, CopyThisLine);   //Axx°
        ...
        fnk_OnBeforeInsertArchSalesLine(ToSalesHeader, ToSalesLine, FromSalesHeaderArchive, FromSalesLineArchive, CopyThisLine);   //Axx°
    ...
    begin
        ...
        if CopyThisLine then begin
            ...
            fnk_OnBeforeInsertArchPurchLine(ToPurchHeader, ToPurchLine, FromPurchHeaderArchive, FromPurchLineArchive, CopyThisLine);   //Axx°
            ToPurchLine.Insert;
    ...
    begin
        with ToSalesHeader do begin
            ...
            "Document Date" := OldSalesHeader."Document Date";
            if not ("Document Type" in ["Document Type"::"Return Order"]) then begin   //Axx°.12
                "Order Date" := OldSalesHeader."Order Date";
            end;   //Axx°.12
            "Promised Delivery Date" := OldSalesHeader."Promised Delivery Date";
            "Shipment Date" := OldSalesHeader."Shipment Date";
            Wiedervorlagedatum := OldSalesHeader.Wiedervorlagedatum;
            "Sachbearbeiter(Telefon)" := OldSalesHeader."Sachbearbeiter(Telefon)";   //C11°
            BonusCreditMemo := OldSalesHeader.BonusCreditMemo;   //B28°
                                                                 //STOP  Axx° ---------------------------------
    ...
    begin
        ...
        if CustLedgEntry.FindFirst then begin
            //START Axx° ---------------------------------
            //Gutschriften sollen nicht automatisch mit Rechnungen ausgeglichen werden
    ...
    begin
        ...
        if DocLineNo = 0 then DocLineNo := FromSalesLine."Line No.";
        ToSalesLine."Attached to Line No." :=
          TransferOldExtLines.TransferExtendedText(DocLineNo, NextLineNo, FromSalesLine."Attached to Line No.");
        //STOP  Axx° ---------------------------------
    end;
    ...
    begin
        ...
        // //START C54° ---------------------------------
        // lo_re_LocationRecordLocation
        // lo_cu_OverallVariableMgtCodeunitOverallVariableMgt
        // lo_cu_WMSManagementCodeunitWMS Management
        // lo_co_BinCodeCode20
        // IF lo_re_Location.GET(ToPurchLine."Location Code") AND (lo_re_Location."Directed Put-away and Pick") AND
        //    lo_cu_WMSManagement.GetDefaultBin(ToPurchLine."No.",ToPurchLine."Variant Code",ToPurchLine."Location Code",lo_co_BinCode) THEN BEGIN
        //  lo_cu_OverallVariableMgt.fnk_SetPurchaseDirectWhsePosting(TRUE);
        //  ToPurchLine.VALIDATE("Bin Code",lo_co_BinCode);
        //  lo_cu_OverallVariableMgt.fnk_ClearPurchaseDirectWhsePosting;
        // END;
        // //STOP  C54° ---------------------------------
    ...
    begin
        ...
        //START Axx° ---------------------------------
        ToSalesLine."Promised Delivery Date" := FromSalesLine."Promised Delivery Date";
        ToSalesLine."Shipment Date" := FromSalesLine."Shipment Date";
        //STOP  Axx° ---------------------------------
        ...
        if ToSalesLine."Document Type" = ToSalesLine."Document Type"::"Return Order" then begin
            if (ToSalesLine.Type = ToSalesLine.Type::Item) and not FromSalesLine.INHMiscItem then begin
                ...
                //    lo_cu_WMSManagementCodeunitWMS Management
                //    lo_co_BinCodeCode20
                //    IF lo_re_Location.GET(ToSalesLine."Location Code") AND (lo_re_Location."Directed Put-away and Pick") AND
                //       lo_cu_WMSManagement.GetDefaultBin(ToSalesLine."No.",ToSalesLine."Variant Code",ToSalesLine."Location Code",lo_co_BinCode) THEN BEGIN
                //      lo_cu_OverallVariableMgt.fnk_SetSalesDirectWhsePosting(TRUE);
                //      ToSalesLine.VALIDATE("Bin Code",lo_co_BinCode);
                //      lo_cu_OverallVariableMgt.fnk_ClearSalesDirectWhsePosting;
                //    END;
                //    //STOP  C54° ---------------------------------
            end;
        end;
        ...
        //START Axx°.7 ---------------------------------
        //Die Standardfunktion(TransferOldExtLines.TransferExtendedText) funktioniert beim kopieren von
        //z.B. geb. Rechnung nicht (CopySalesInvLinesToDoc ändert LineNo)
        if (ToSalesLine."Attached to Line No." = 0) and (FromSalesLine."Attached to Line No." <> 0) then begin
            if re_RealFromLineNoBuffer.Get(FromSalesLine."Attached to Line No.") then begin
                if re_LineNoBuffer.Get(re_RealFromLineNoBuffer."New Line Number") then begin
                    ToSalesLine."Attached to Line No." := re_LineNoBuffer."New Line Number";
                end;
            end;
        end;
        //STOP  Axx°.7 ---------------------------------
        ...
        //START B17° ---------------------------------
        re_LineNoBuffer."Old Line Number" := FromSalesLine."Line No.";
        re_LineNoBuffer."New Line Number" := ToSalesLine."Line No.";
        re_LineNoBuffer.Insert;
        if CopyThisLine then begin
            if FromSalesLine.Gruppenpreis > 0 then begin
                re_LineNoBufferGroupPrice."Old Line Number" := FromSalesLine."Line No.";
                re_LineNoBufferGroupPrice."New Line Number" := ToSalesLine."Line No.";
                re_LineNoBufferGroupPrice.Insert;
            end;
        end;
        //STOP  B17° ---------------------------------
    ...
    begin
        ...
        //START Axx° ---------------------------------
        ToSalesLine."Promised Delivery Date" := FromArchSalesLine."Promised Delivery Date";
        ToSalesLine."Shipment Date" := FromArchSalesLine."Shipment Date";
        //STOP  Axx° ---------------------------------
        ...
        //START B17° ---------------------------------
        re_LineNoBuffer."Old Line Number" := FromArchSalesLine."Line No.";
        re_LineNoBuffer."New Line Number" := ToSalesLine."Line No.";
        re_LineNoBuffer.Insert;
        if CopyThisLine then begin
            if FromArchSalesLine.Gruppenpreis > 0 then begin
                re_LineNoBufferGroupPrice."Old Line Number" := FromArchSalesLine."Line No.";
                re_LineNoBufferGroupPrice."New Line Number" := ToSalesLine."Line No.";
                re_LineNoBufferGroupPrice.Insert;
            end;
        end;
    ...
    var
        lo_re_StandardText: Record "Standard Text";
    ...
    begin
        ...
        if (FromSalesLine.Type = FromSalesLine.Type::Item) and (FromSalesLine."No." = '') then begin
            ...
            ToSalesLine."Document Type" := ToSalesHeader."Document Type";
            ToSalesLine."Document No." := ToSalesHeader."No.";
            ToSalesLine."Line No." := lo_in_LineNo;
            ToSalesLine.Type := ToSalesLine.Type::Item;
            ToSalesLine.Description := FromSalesLine.Description;
            ToSalesLine."Description 2" := FromSalesLine."Description 2";
            ToSalesLine."Attached to Line No." := FromSalesLine."Attached to Line No.";
            ToSalesLine."Location Code" := FromSalesLine."Location Code";
        end;
        ...
        if (FromSalesLine.Type = FromSalesLine.Type::Item) and (FromSalesLine."No." = '') then begin
            //START Axx°.7 ---------------------------------
            if FromSalesLine."Attached to Line No." <> 0 then begin
                if re_RealFromLineNoBuffer.Get(FromSalesLine."Attached to Line No.") then begin
                    if re_LineNoBuffer.Get(re_RealFromLineNoBuffer."New Line Number") then begin
                        ToSalesLine."Attached to Line No." := re_LineNoBuffer."New Line Number";
                    end;
        ...
        if (FromSalesLine.Type = FromSalesLine.Type::" ") and (FromSalesLine."No." <> '') then begin
            if not lo_re_StandardText.Get(FromSalesLine."No.") then begin
                FromSalesLine."No." := '';
            end;
        end;
        //STOP  Axx°.16 ---------------------------------
        ...
        if (RecalculateLines) and (not FromSalesLine.INHMiscItem) then begin
            ...
            if FromSalesLine.INHMiscItem and (FromSalesLine."No." <> '') then begin
                ...
                if (ToSalesLine."Gen. Bus. Posting Group" <> lo_re_ToSalesLinePrev."Gen. Bus. Posting Group") or
                   ...
                   (ToSalesLine."VAT Bus. Posting Group" <> lo_re_ToSalesLinePrev."VAT Bus. Posting Group") or
                   (ToSalesLine."VAT Prod. Posting Group" <> lo_re_ToSalesLinePrev."VAT Prod. Posting Group")
                then begin
                    if lo_re_ToSalesLinePrev."Gen. Bus. Posting Group" <> '' then begin
                        ToSalesLine."Gen. Bus. Posting Group" := lo_re_ToSalesLinePrev."Gen. Bus. Posting Group";
                    end;
                    if lo_re_ToSalesLinePrev."Gen. Prod. Posting Group" <> '' then begin
                        ToSalesLine."Gen. Prod. Posting Group" := lo_re_ToSalesLinePrev."Gen. Prod. Posting Group";
                    end;
                    //START C92° ---------------------------------
    ...
    begin
        ...
        if (RecalculateLines) and (not FromArchSalesLine.INHMiscItem) then begin
            ...
            if (FromArchSalesLine.Type = FromArchSalesLine.Type::" ") and (FromArchSalesLine."No." <> '') then begin
                ...
                ToSalesLine.Validate("Description 2", FromArchSalesLine."Description 2");
            end;
            ...
            ToSalesLine."Installation Group" := FromArchSalesLine."Installation Group";   //B74°
            ToSalesLine."Shipment Date Group" := FromArchSalesLine."Shipment Date Group";   //B75°
            ToSalesLine.Artikeltyp := FromArchSalesLine.Artikeltyp;
            ToSalesLine."Pos." := FromArchSalesLine."Pos.";
            ...
            if (ToSalesHeader."Document Type" in
               [ToSalesHeader."Document Type"::"Return Order", ToSalesHeader."Document Type"::"Credit Memo"])
            then begin
                ToSalesLine."Job No." := FromArchSalesLine."Job No.";
                ToSalesLine."Job Task No." := FromArchSalesLine."Job Task No.";
            end;
            //STOP  Axx° ---------------------------------
            ...
            if FromArchSalesLine.INHMiscItem and (FromArchSalesLine."No." <> '') then begin
                ...
                ToSalesLine.Validate("Unit of Measure Code", FromArchSalesLine."Unit of Measure Code");
                ToSalesLine.Validate(Quantity, FromArchSalesLine.Quantity);
                ToSalesLine.Validate("Unit Price", FromArchSalesLine."Unit Price");
                ToSalesLine.Validate("Unit Cost (LCY)", FromArchSalesLine."Unit Cost (LCY)");
                ToSalesLine.Kurzbezeichnung := FromArchSalesLine.Kurzbezeichnung;
                ToSalesLine."Pos." := FromArchSalesLine."Pos.";
                ToSalesLine.Validate("Preis-KZ", FromArchSalesLine."Preis-KZ");
                ToSalesLine.Validate(Preisherkunft, FromArchSalesLine.Preisherkunft);
                //START A97°.1 ---------------------------------
                if (ToSalesLine."Gen. Bus. Posting Group" <> lo_re_ToSalesLinePrev."Gen. Bus. Posting Group") or
                   (ToSalesLine."Gen. Prod. Posting Group" <> lo_re_ToSalesLinePrev."Gen. Prod. Posting Group") or
                   (ToSalesLine."VAT Bus. Posting Group" <> lo_re_ToSalesLinePrev."VAT Bus. Posting Group") or
                ...
                //STOP  A97°.1 ---------------------------------
                ToSalesLine.Validate("Special Posting Group Mode", FromArchSalesLine."Special Posting Group Mode");   //B85°
                ToSalesLine.Description := FromArchSalesLine.Description;
                ToSalesLine."Description 2" := FromArchSalesLine."Description 2";
                ToSalesLine."Vendor Item No." := FromArchSalesLine."Vendor Item No.";   //C47°
            end;

            //START C92° ---------------------------------
            if FromArchSalesLine.Type = FromArchSalesLine.Type::Item then begin
                if lo_re_VATPostingSetup.Get(ToSalesLine."VAT Bus. Posting Group", ToSalesLine."VAT Prod. Posting Group") then;
                if (lo_re_VATPostingSetup."OPP Valid Until" <> 0D) and (lo_re_VATPostingSetup."OPP Valid Until" <= WorkDate) then begin
                    if lo_re_Item.Get(FromArchSalesLine."No.") then begin
            ...
            if (FromArchSalesLine.Type = FromArchSalesLine.Type::Item) and (FromArchSalesLine."No." <> '') then begin
                //TODO: Funktion müsste für Archiv angepasst werden bei Bedarf
                //lo_cu_CalcMgt.FNK_KalkulationVonVKBelegZuVKB(FromSalesLine,ToSalesLine);
                lo_cu_CalcMgt.FNK_KalkulaRabatteZuVKZeileAkt(ToSalesLine);
            end;
            ...
            if ToSalesLine.Positionsart = ToSalesLine.Positionsart::"gelöscht mit Aufpreisposition" then begin
                ToSalesLine."Special Order Purch. Line No." := ToSalesLine."Line No." + 10000;
                ToSalesLine."Line Amount" := FromArchSalesLine."Line Amount";
                bo_AufpreisLineNo := true;
            end else
                if (ToSalesLine.Positionsart = ToSalesLine.Positionsart::Aufpreisposition) and (not bo_AufpreisLineNo) then begin
                    ToSalesLine."Special Order Purch. Line No." := ToSalesLine."Line No.";
    ...
    local procedure fnk_WriteOrderInfoLines(par_re_ToSalesHeader: Record "Sales Header"; var par_re_FromSalesInvLine: Record "Sales Invoice Line"; var par_re_FromSalesLineBuf: Record "Sales Line" temporary; var par_in_NextLineNo: Integer)
    var
        lo_re_SalesHdr: Record "Sales Header";
        lo_re_SalesHdrArch: Record "Sales Header Archive";
    begin
        // *** Fügt dem Buffer Zeilen hinzu mit allgemeinen Infos zu einer Rechnungszeile bezogen auf den Auftrag ***   //Axx°.2
        Clear(par_re_FromSalesLineBuf);
        par_in_NextLineNo += 1;
        par_re_FromSalesLineBuf."Document No." := par_re_ToSalesHeader."No.";
        par_re_FromSalesLineBuf."Shipment No." := par_re_FromSalesInvLine."Document No.";
        par_re_FromSalesLineBuf.Type := par_re_FromSalesLineBuf.Type::" ";

        par_re_FromSalesLineBuf."Line No." := par_in_NextLineNo;
        par_re_FromSalesLineBuf.Description := '';
        par_re_FromSalesLineBuf."Description 2" := '';
        par_re_FromSalesLineBuf.Insert;
        par_in_NextLineNo += 1;

        par_re_FromSalesLineBuf."Line No." := par_in_NextLineNo;
        par_re_FromSalesLineBuf.Description := StrSubstNo(TextShipNo, par_re_FromSalesInvLine.IC_Lieferschein);
        par_re_FromSalesLineBuf."Description 2" := 'vom ' + Format(par_re_FromSalesInvLine."Shipment Date");
        par_re_FromSalesLineBuf.Insert;
        par_in_NextLineNo += 1;

        par_re_FromSalesLineBuf."Line No." := par_in_NextLineNo;
        par_re_FromSalesLineBuf.Description := StrSubstNo(TextOrderNo, par_re_FromSalesInvLine.Auftragsnummer);
        par_re_FromSalesLineBuf."Description 2" := '';
        par_re_FromSalesLineBuf.Insert;
        par_in_NextLineNo += 1;

        if not lo_re_SalesHdr.Get(lo_re_SalesHdr."Document Type"::Order, par_re_FromSalesInvLine.Auftragsnummer) then begin
            lo_re_SalesHdrArch.SetRange("Document Type", lo_re_SalesHdr."Document Type"::Order);
            lo_re_SalesHdrArch.SetRange("No.", par_re_FromSalesInvLine.Auftragsnummer);
            if lo_re_SalesHdrArch.FindLast then begin
                lo_re_SalesHdr."Kom." := lo_re_SalesHdrArch."Kom.";
                lo_re_SalesHdr."Your Reference" := lo_re_SalesHdrArch."Your Reference";
                lo_re_SalesHdr."Ihre Bestellnummer" := lo_re_SalesHdrArch."Ihre Bestellnummer";
            end;
        end;

        if lo_re_SalesHdr."Kom." <> '' then begin
            par_re_FromSalesLineBuf."Line No." := par_in_NextLineNo;
            par_re_FromSalesLineBuf.Description := TextKom;
            par_re_FromSalesLineBuf."Description 2" := lo_re_SalesHdr."Kom.";
            par_re_FromSalesLineBuf.Insert;
            par_in_NextLineNo += 1;
        end;

        if lo_re_SalesHdr."Your Reference" <> '' then begin
            par_re_FromSalesLineBuf."Line No." := par_in_NextLineNo;
            par_re_FromSalesLineBuf.Description := lo_re_SalesHdr.FieldCaption("Your Reference") + ':';
            par_re_FromSalesLineBuf."Description 2" := lo_re_SalesHdr."Your Reference";
            par_re_FromSalesLineBuf.Insert;
            par_in_NextLineNo += 1;
        end;

        if lo_re_SalesHdr."Ihre Bestellnummer" <> '' then begin
            par_re_FromSalesLineBuf."Line No." := par_in_NextLineNo;
            par_re_FromSalesLineBuf.Description := lo_re_SalesHdr.FieldCaption("Ihre Bestellnummer") + ':';
            par_re_FromSalesLineBuf."Description 2" := lo_re_SalesHdr."Ihre Bestellnummer";
            par_re_FromSalesLineBuf.Insert;
            par_in_NextLineNo += 1;
        end;
    end;
