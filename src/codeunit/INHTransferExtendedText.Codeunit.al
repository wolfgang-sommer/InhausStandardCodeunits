codeunit 50143 INHTransferExtendedText
{
    // +---------------------------------------------+
    // +                                             +
    // +           Inhaus Handels GmbH               +
    ...
    // ------------------------------------------------------------
    // Axx°          RBI  28.07.08  Anpassungen in CU übernommen.
    //               RBI  03.06.09  Lieferantenartikelbezeichnung und zusätzlich die Artikelbeschreibung übernehmen.
    // Axx°.1        SSC  28.06.12  Fehlermeldung Text000 erweitert um die Nr.
    // Axx°.2        SSC  12.08.16  Neue fnk_SetMaxLineSpacing
    // Axx°.3        SSC  01.03.19  Bug: Wenn Zeile selbst verknüpft ist(z.B. Montage) dann wird TBS nicht gelöscht aber neu angelegt => doppelt
    // B44°          SSC  16.01.12  Hier keine verknüpften Variantentexte löschen
    ...
    var
        Text000: Label 'There is not enough space to insert extended text lines. Nr.: %1.';
        ...
        "+++VAR_INHAUS+++": Boolean;
        TextLines: Integer;
        IN_AttachedToLine: Integer;
        in_MaxLineSpacing: Integer;
    ...
    begin
        ...
        if (SalesLine."Line No." <> 0) and (SalesLine."No." <> '') then
            //STOP  Axx°.3 ---------------------------------
    ...
    begin
        ...
        if ToSalesLine.Find('>') then begin
            ...
            if LineSpacing = 0 then
                Error(Text000, SalesLine."No.");   //Axx°.1
        end else
            LineSpacing := 10000;

        if (in_MaxLineSpacing > 0) and (in_MaxLineSpacing < LineSpacing) then LineSpacing := in_MaxLineSpacing;   //Axx°.2
    ...
    var
        ...
        LO_RE_ArtikelLieferantKatalog: Record "Item Vendor";
    ...
    begin
        ...
        //START Axx° ---------------------------------
        IN_AttachedToLine := PurchLine."Line No.";
        LO_RE_ArtikelLieferantKatalog.Reset;
        LO_RE_ArtikelLieferantKatalog.SetRange("Vendor No.", PurchLine."Buy-from Vendor No.");
        LO_RE_ArtikelLieferantKatalog.SetRange("Item No.", PurchLine."No.");
        LO_RE_ArtikelLieferantKatalog.SetFilter(Lieferantenartikelbezeichnung, '<>%1', '');
        if LO_RE_ArtikelLieferantKatalog.FindFirst then begin
            ToPurchLine.Init;
            ToPurchLine."Document Type" := PurchLine."Document Type";
            ToPurchLine."Document No." := PurchLine."Document No.";
            ToPurchLine."Buy-from Vendor No." := PurchLine."Buy-from Vendor No.";
            ToPurchLine."Line No." := NextLineNo;
            NextLineNo := NextLineNo + LineSpacing;
            ToPurchLine.Type := 0;
            LO_RE_Item.Get(PurchLine."No.");
            ToPurchLine.Description := LO_RE_Item.Description;
            ToPurchLine."Description 2" := LO_RE_Item."Description 2";
            ToPurchLine."Attached to Line No." := PurchLine."Line No.";
            ToPurchLine.IC_AuftragZeile := PurchLine."Line No.";
            ToPurchLine.IC_Mandant := PurchLine.IC_Mandant;
            ToPurchLine.IC_Typ := PurchLine.IC_Typ;
            ToPurchLine.Insert;
        end;
        //STOP  Axx° ---------------------------------
    ...
    var
        ...
        "+++LO_VAR_INHAUS+++": Boolean;
        LO_RE_PurchaseLine: Record "Purchase Line";
    begin
        ...
        SalesLine2.SetRange(Artikeltyp, SalesLine2.Artikeltyp::" ");   //Axx°
        SalesLine2.SetFilter("Attached to - Type", '<>%1', SalesLine2."Attached to - Type"::Variant);  // B44°
        SalesLine2 := SalesLine;
        if SalesLine2.Find('>')
          and (SalesLine2.Type = SalesLine2.Type::" ")   //Axx°
        then begin
            repeat

                //START Axx° ------------------------------------------------
                // Löscht bei IC-Aufträgen die zugehörige Bestellzeile
                if (SalesLine2.IC_Typ = SalesLine2.IC_Typ::Auftrag) then begin
                    LO_RE_PurchaseLine.SetCurrentKey("Document Type", "Document No.");
                    LO_RE_PurchaseLine.SetRange("Document Type", LO_RE_PurchaseLine."Document Type"::Order);
                    LO_RE_PurchaseLine.SetRange("Document No.", SalesLine2.IC_Bestellung);
                    LO_RE_PurchaseLine.SetRange("Line No.", SalesLine2.IC_BestellZeile);
                    if LO_RE_PurchaseLine.FindFirst then begin
                        LO_RE_PurchaseLine.Delete;
                    end;
                end;
                //STOP Axx° ---------------------------------------------------
    ...
    [Scope('Internal')]
    procedure fnk_SetMaxLineSpacing(par_in_MaxLineSpacing: Integer)
    begin
        //Axx°.2
        in_MaxLineSpacing := par_in_MaxLineSpacing;
    end;
}
