codeunit 50150 INHUndoSalesShipmentLine
{
    // +---------------------------------------------+
    // +                                             +
    // +           Inhaus Handels GmbH               +
    ...
    // ------------------------------------------------------------
    // Axx°          RBI  05.02.09  Bei Lieferung stornieren das Buchungsdatum auf WORKDATE setzen.
    // C23°          SSC  02.03.16  IC-Entkoppelt; Bei IC-Aufträgen gibt es keinen VK-Beleg im anderen Mandanten
    // C27°          SSC  09.03.18  Hooks
    ...
    begin
        ...
        if IsHandled then
            exit;
        lo_cu_SalesMgt.fnk_Cu5815_OnBeforeOnRun(Rec);   //C27°
        ...
        lo_cu_SalesMgt.fnk_Cu5815_OnAfterOnRun(Rec);   //C27°
    end;
        ...
        with SalesShptLine do begin
            ...
            if GetUnvoicedShptEntries(SalesShptLine, ItemLedgEntryNotInvoiced) then begin
                repeat
                    ...
                    lo_cu_ICMgt.fnk_B4UndoShptItemJnlPost(ItemJnlLine, SalesShptLine);   //C23°
