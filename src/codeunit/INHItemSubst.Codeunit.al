codeunit 50123 INHItemSubst
{
    ...
    // ------------------------------------------------------------
    // Axx°          SSC  06.11.08  Ersatzartikel mit der neuen Verfügbarkeit berechnen und den Lagerbestand in der Form anzeigen
    // Axx°.1        GPET 25.07.23  Fehler Ersatzartikel andere Einheit -> nicht die ursprüngliche Einheit übernehmen
    // A96°          SSC  25.02.10  Bei Auslaufartikel die verfügbar sind, kein Ersatzartikelvorschlag
    //                              Wenn nicht verfügbar, muss Ersatzartikel ausgewählt werden
    // B29°          SSC  18.10.17  Keine Auswahl bringen wenn NOT GUIALLOWED
    ...
    begin
        ...
        if ItemSubstitution.Find('-') then begin
            ...
            then begin
                ...
                if lo_bo_DiscontinuedItem then begin
                    Error(TextDiscItem);
                end;
                //STOP  A96° ---------------------------------
            end;
    ...
    [Scope('Internal')]
    procedure fnk_SetxSalesLine(par_re_xSalesLine: Record "Sales Line")
    begin
        //A96°
        re_xSalesLine := par_re_xSalesLine;
    end;
