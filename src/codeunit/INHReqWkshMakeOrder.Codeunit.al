codeunit 50167 INHReqWkshMakeOrder
{
    // +---------------------------------------------+
    // +                                             +
    // +           Inhaus Handels GmbH               +
    ...
    // ------------------------------------------------------------
    // Axx°          RBI  28.07.08  Anpassungen in CU übernehmen
    // Axx°.1        SSC  22.05.18  DialogFenster nicht öffnen, da es (teilweise) nicht geschlossen wird, auch Windows.CLOSE hilft nicht???
    // Axx°.2        SSC  11.07.18  Gesperrte Kreditoren nicht zulassen
    // Axx°.3        SSC  19.10.18  Prüfung auf "doppelte" Bestellung, da es hin und wieder passiert, dass Bestellung erstellt wird aber der
    //                               Bestellvorschlag bestehen bleibt
    // Axx°.4        SSC  05.04.19  Den Preis von der Bestellvorschlagszeile nicht in Bestellung übernehmen
    // B67°          SSC  20.02.13  Bestellung pro Warenempfängernr.
    // B67°.2        SSC  12.07.22  Standardlagerplatz als primäres Kriterium
    ...
    var
        ...
        bo_IsGoodsRcptGroupOrder: Boolean;
        "+++TE_INHAUS+++": ;
        TextAlreadyExisting: Label 'Achtung. In %1 befinden sich bereits %2 von %3 der Artikel dieses Vorschlages.\Möchten sie trotzdem fortfahren?';
        TextCancel: Label 'Abbortion';
    ...
    begin
        ...
        with ReqLine do begin
            ...
            repeat
                ...
                //Axx°.1:Window.UPDATE(2,LineCount);
            ...
            //START B67° ---------------------------------
            if co_MainOrderNo <> '' then begin
                cu_PurchMgt.fnk_FindConnGoodsRcptGrpOrders(co_MainOrderNo);
                cu_PurchMgt.fnk_MakeGoodsRcptComments(co_MainOrderNo);
            end;
            //STOP  B67° ---------------------------------
            cu_PurchMgt."FNK_Poolen_Abschlußzeilen"(PurchOrderLine, NextLineNo);   //Axx°
    ...
    begin
        with RequisitionLine do begin
            ...
            //Axx°:PurchOrderLine.VALIDATE("Line Discount %","Line Discount %");
    ...
    begin
        ...
        with ReqLine2 do begin
            ...
            //START B67° ---------------------------------
            if cu_PurchMgt.fnk_UsesGoodsRcptGroups(ReqLine2."Vendor No.", true) then begin
                bo_IsGoodsRcptGroupOrder := true;
                //START B67°.2 ---------------------------------
                //lo_co_ShelfNo := lo_cu_ItemMgt.FNK_RegalnummerZuArtikel(ReqLine2."No.",WORKDATE,ReqLine2."Location Code");
                lo_co_ShelfNo := lo_cu_ItemMgt.fnk_GetBinCodeOrShelfNo(ReqLine2."No.", WorkDate, ReqLine2."Location Code");
                //STOP  B67°.2 ---------------------------------
                co_GoodsRcptGroup := cu_PurchMgt.fnk_GetGoodsRcptGroup(ReqLine2."Vendor No.", lo_co_ShelfNo);
            end;
            if (co_MainOrderNo <> '') and (PurchOrderHeader."Goods Rcpt. Group" <> co_GoodsRcptGroup) then begin
                PurchOrderHeader.Reset;
                PurchOrderHeader.SetCurrentKey("Document Type", "Buy-from Vendor No.", Status);
                PurchOrderHeader.SetRange("Document Type", PurchOrderHeader."Document Type"::Order);
                PurchOrderHeader.SetRange("Buy-from Vendor No.", "Vendor No.");
                PurchOrderHeader.SetRange(Status, PurchOrderHeader.Status::Open);
                PurchOrderHeader.SetRange("Location Code", "Location Code");
                PurchOrderHeader.SetRange("Goods Rcpt. Group", co_GoodsRcptGroup);
                PurchOrderHeader.SetRange("Main Order No.", co_MainOrderNo);
                if PurchOrderHeader.FindFirst then begin
                    ...
                    lo_bo_MakeNewOrder := true;
                end;
            end;
            //STOP  B67° ---------------------------------

            if CheckInsertFinalizePurchaseOrderHeader(ReqLine2, PurchOrderHeader, true)
              or lo_bo_MakeNewOrder   //B67°
            then begin
                InsertHeader(ReqLine2);
                LineCount := 0;
                if (PurchOrderHeader."Bestell-KZ" <> PurchOrderHeader."Bestell-KZ"::Poolen) then   //Axx°
                    NextLineNo := 0;
            ...
            //START Axx° ---------------------------------
            PurchOrderLine.IC_Typ := PurchOrderHeader.IC_Typ;
            if (PurchOrderLine.IC_Typ = PurchOrderLine.IC_Typ::BVO) then begin
                Validate("Due Date", CalcDate('1T', WorkDate));
                PurchOrderLine.Validate("Expected Receipt Date", "Due Date");
                PurchOrderLine."Requested Receipt Date" := PurchOrderLine."Expected Receipt Date";
    ...
    var
        ...
        "+++LO_VAR_INHAUS+++": Boolean;
        lo_re_ReqLine: Record "Requisition Line";
        lo_in_FoundCount: Integer;
    begin
        ...
        with ReqLine2 do begin
            ...
            //START Axx°.2 ---------------------------------
            Vendor.Get(ReqLine2."Vendor No.");
            Vendor.TestField(Blocked, Vendor.Blocked::" ");
            if ReqLine2."Vendor No." <> ReqLine2."Item Vendor No." then begin
                Vendor.Get(ReqLine2."Item Vendor No.");
                Vendor.TestField(Blocked, Vendor.Blocked::" ");
            end;
            //STOP  Axx°.2 ---------------------------------
            ...
            //START Axx° ---------------------------------
            PurchOrderHeader.Reset;
            PurchOrderHeader.SetCurrentKey("Document Type",
                                           "Buy-from Vendor No.",
                                           Status,
                                           "Bestell-KZ");
            PurchOrderHeader.SetRange("Document Type", PurchOrderHeader."Document Type"::Order);
            PurchOrderHeader.SetRange("Buy-from Vendor No.", "Vendor No.");
            PurchOrderHeader.SetRange(Status, PurchOrderHeader.Status::Open);
            PurchOrderHeader.SetRange("Bestell-KZ", PurchOrderHeader."Bestell-KZ"::Poolen);
            PurchOrderHeader.SetRange("Location Code", "Location Code");
            //START A67° ---------------------------------
            if bo_IsGoodsRcptGroupOrder then begin
                PurchOrderHeader.SetRange("Goods Rcpt. Group", co_GoodsRcptGroup);
                PurchOrderHeader.SetFilter("Main Order No.", '%1|%2', '', co_MainOrderNo);
            end;
            //STOP  A67° ---------------------------------
            if not PurchOrderHeader.FindFirst then begin
                //STOP  Axx° ---------------------------------
                ...
                //Axx°:PurchOrderHeader."Expected Receipt Date" := ReceiveDateReq;
                ...
                //START Axx° ---------------------------------
                PurchOrderHeader."Bestell-KZ" := PurchOrderHeader."Bestell-KZ"::Poolen;
                NextLineNo := 0;
                Vendor.Get(PurchOrderHeader."Buy-from Vendor No.");
                PurchOrderHeader."Purchaser Code" := Vendor."Purchaser Code";

                re_InitTable.Get(CompanyName);
                if (PurchOrderHeader."Buy-from Vendor No." = re_InitTable.IC_Vendor) then begin
                    PurchOrderHeader.IC_Typ := PurchOrderHeader.IC_Typ::BVO;
                    PurchOrderHeader.IC_Bestellung := PurchOrderHeader."No.";
                    PurchOrderHeader.IC_Mandant := CompanyName;
                    PurchOrderHeader."Purchaser Code" := UserId;
                    if PurchOrderHeader."Expected Receipt Date" = 0D then begin
                        PurchOrderHeader.Validate("Expected Receipt Date", PurchOrderHeader."Posting Date");
                    end;
                end;
                //STOP  Axx° ---------------------------------

                //START Axx° ---------------------------------
                //offene, gepoolte Bestellung suchen
            end else begin
                NextLineNo := 0;
                PurchOrderLine.Reset;
                PurchOrderLine.SetRange("Document Type", PurchOrderHeader."Document Type");
                ...
                if GuiAllowed then begin
                    lo_re_ReqLine.CopyFilters(ReqLine2);
                    if lo_re_ReqLine.FindSet(false, false) then begin
                        PurchOrderLine.Reset;
                        PurchOrderLine.SetRange("Document Type", PurchOrderHeader."Document Type");
                        PurchOrderLine.SetRange("Document No.", PurchOrderHeader."No.");
                        repeat
                            PurchOrderLine.SetRange("No.", lo_re_ReqLine."No.");
                            if not PurchOrderLine.IsEmpty then begin
                                lo_in_FoundCount += 1;
                            end;
                        until lo_re_ReqLine.Next = 0;
                        if lo_in_FoundCount >= (lo_re_ReqLine.Count * 0.5) then begin
                            if not Confirm(TextAlreadyExisting, false, PurchOrderHeader."No.", lo_in_FoundCount, lo_re_ReqLine.Count) then begin
                                Error(TextCancel);
                            end;
                        end;
                    end;
            ...
            //START B67° ---------------------------------
            if bo_IsGoodsRcptGroupOrder then begin
                PurchOrderHeader."Goods Rcpt. Group" := co_GoodsRcptGroup;
                if co_MainOrderNo = '' then begin
                    co_MainOrderNo := PurchOrderHeader."No.";
                end;
                PurchOrderHeader."Main Order No." := co_MainOrderNo;
            end;
            //STOP  B67° ---------------------------------
