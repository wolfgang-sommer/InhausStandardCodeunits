codeunit 50148 INHUndoReturnReceiptLine
{
    // +---------------------------------------------+
    // +                                             +
    // +           Inhaus Handels GmbH               +
    ...
    // ------------------------------------------------------------
    // Axx°          RBI  05.02.09  Bei Rücklieferung stornieren das Buchungsdatum auf WORKDATE setzen.
    //               RBI  11.03.09  Sachkonten auch stornieren
    // C27°          SSC  10.01.20  OnBeforeUndoRetRcptLine
    ...
    begin
        OnBeforeUndoRetRcptLine(ReturnRcptLine);   //C27°
    ...
    [IntegrationEvent(false, false)]
    local procedure OnBeforeUndoRetRcptLine(var ReturnRcptLine: Record "Return Receipt Line")
    begin
    end;
}
