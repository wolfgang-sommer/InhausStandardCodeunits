codeunit 50141 INHServOrderManagement
{
    // +---------------------------------------------+
    // +                                             +
    // +           Inhaus Handels GmbH               +
    ...
    // ------------------------------------------------------------
    // C40°          SSC  30.01.19  Permissions TableData Customer=rim
        ...
        repeat
            ...
            if ServHour.FindLast then begin
                ...
                if not Holiday or ServHour."Valid on Holidays" then begin
                    ...
                    if StartingDate = FinishingDate then
                        TotTime := CalendarMgmt.CalcTimeDelta(FinishingTime,
                            GetStartingTime(StartingTime, ServHour."Starting Time"))
                    else
                        case TempDate of
                            StartingDate:
                                if ServHour."Ending Time" > StartingTime then
                                    TotTime := TotTime +
                                      CalendarMgmt.CalcTimeDelta(ServHour."Ending Time",
                                        GetStartingTime(StartingTime, ServHour."Starting Time"));
                            FinishingDate:
