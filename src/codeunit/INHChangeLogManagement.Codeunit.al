codeunit 50177 INHChangeLogManagement
{
    // +---------------------------------------------+
    // +                                             +
    // +           Inhaus Handels GmbH               +
    ...
    // ------------------------------------------------------------
    // B81°          SSC  18.06.15  Neue fnk_InsertLogEntryRecRef, fnk_InsertLogEntryT27
    //               SSC  16.11.20  Neue fnk_InsertLogEntryT39, fnk_InsertLogEntryT246
    ...
    [Scope('Internal')]
    procedure fnk_InsertLogEntryRecRef(par_rf_RecRef: RecordRef; par_rf_xRecRef: RecordRef; par_bo_All: Boolean; par_in_FieldNo: Integer; par_op_TypeOfChange: Option Insertion,Modification,Deletion)
    var
        lo_fr_FldRef: FieldRef;
        lo_fr_xFldRef: FieldRef;
    begin
        // *** Eintrag in Änderungsprotokoll   //B81°
        if par_bo_All then begin
            case par_op_TypeOfChange of
                par_op_TypeOfChange::Insertion:
                    LogInsertion(par_rf_RecRef);
                par_op_TypeOfChange::Modification:
                    LogModification(par_rf_RecRef);
                par_op_TypeOfChange::Deletion:
                    LogDeletion(par_rf_RecRef);
            end;
        end else begin
            lo_fr_FldRef := par_rf_RecRef.Field(par_in_FieldNo);
            lo_fr_xFldRef := par_rf_xRecRef.Field(par_in_FieldNo);
            InsertLogEntry(lo_fr_FldRef, lo_fr_xFldRef, par_rf_RecRef, par_op_TypeOfChange, true);
        end;
    end;

    [Scope('Internal')]
    procedure fnk_InsertLogEntryT27(par_re_Item: Record Item; par_re_xItem: Record Item; par_bo_All: Boolean; par_in_FieldNo: Integer; par_op_TypeOfChange: Option Insertion,Modification,Deletion)
    var
        lo_rf_RecRef: RecordRef;
        lo_rf_xRecRef: RecordRef;
    begin
        // *** Eintrag in Änderungsprotokoll - Artikel   //B81°
        lo_rf_RecRef.GetTable(par_re_Item);
        lo_rf_xRecRef.GetTable(par_re_xItem);
        fnk_InsertLogEntryRecRef(lo_rf_RecRef, lo_rf_xRecRef, par_bo_All, par_in_FieldNo, par_op_TypeOfChange);
    end;

    [Scope('Internal')]
    procedure fnk_InsertLogEntryT39(par_re_PurchLine: Record "Purchase Line"; par_re_xPurchLine: Record "Purchase Line"; par_in_FieldNo: Integer; par_op_TypeOfChange: Option Insertion,Modification,Deletion)
    var
        lo_rf_RecRef: RecordRef;
        lo_rf_xRecRef: RecordRef;
        lo_fr_FldRef: FieldRef;
        lo_fr_xFldRef: FieldRef;
    begin
        // *** Eintrag in Änderungsprotokoll - EK-Zeile   //B81°

        lo_rf_RecRef.GetTable(par_re_PurchLine);
        lo_rf_xRecRef.GetTable(par_re_xPurchLine);

        lo_fr_FldRef := lo_rf_RecRef.Field(par_in_FieldNo);
        lo_fr_xFldRef := lo_rf_xRecRef.Field(par_in_FieldNo);

        InsertLogEntry(lo_fr_FldRef, lo_fr_xFldRef, lo_rf_RecRef, par_op_TypeOfChange, true);
    end;

    [Scope('Internal')]
    procedure fnk_InsertLogEntryT246(par_re_ReqLine: Record "Requisition Line"; par_re_xReqLine: Record "Requisition Line"; par_in_FieldNo: Integer; par_op_TypeOfChange: Option Insertion,Modification,Deletion)
    var
        lo_rf_RecRef: RecordRef;
        lo_rf_xRecRef: RecordRef;
        lo_fr_FldRef: FieldRef;
        lo_fr_xFldRef: FieldRef;
    begin
        // *** Eintrag in Änderungsprotokoll - Bestellvorschlagszeile   //B81°

        lo_rf_RecRef.GetTable(par_re_ReqLine);
        lo_rf_xRecRef.GetTable(par_re_xReqLine);
