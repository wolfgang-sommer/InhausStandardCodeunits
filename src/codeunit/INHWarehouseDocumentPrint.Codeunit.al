codeunit 50150 INHWarehouseDocumentPrint
{
    ...
    begin
        ...
        fnk_PrintPickHeader(WhseActivHeader);
        //STOP  A08° ---------------------------------
    end;
    ...
    local procedure fnk_PrintPickHeader(WhseActivHeader: Record "Warehouse Activity Header")
    var
        lo_re_WhseActLine: Record "Warehouse Activity Line";
        lo_re_WhseShptHdr: Record "Warehouse Shipment Header";
    begin
        //A08°
        lo_re_WhseActLine.SetRange("No.", WhseActivHeader."No.");
        if lo_re_WhseActLine.Find('-') then begin
            lo_re_WhseShptHdr.SetRange("No.", lo_re_WhseActLine."Whse. Document No.");
            if lo_re_WhseShptHdr.Find('-') then begin
                PrintShptHeader(lo_re_WhseShptHdr);
            end;
        end;
    end;
}
