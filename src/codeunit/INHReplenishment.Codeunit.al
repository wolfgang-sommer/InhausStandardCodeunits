codeunit 50132 INHReplenishment
{
    // +---------------------------------------------+
    // +                                             +
    // +           Inhaus Handels GmbH               +
    ...
    // ------------------------------------------------------------
    // C54°          SSC  14.10.20  Bestehende Vorschlagszeilen auch berücksichtigen, sonst werden sie mehrfach angelegt
    //                              Andere Idee mit dem zusätzlichen füllen von TempWhseWkshLine funktioniert nicht, da diese später eingefügt werden in Funktion InsertWhseWkshLine
    //                              Löschen der Zeilen vorab wäre auch möglich ist aber nicht ideal
    //               SSC  28.01.21  Fälligkeitsdatum füllen
    ...
    begin
        with ToBinContent do begin
            ...
            fnk_IncludeExistingWhseWkstLinesInRemainQtyToReplenish(RemainQtyToReplenishBase, ToBinContent, WhseWkshTemplateName, WhseWkshName);   //C54°
            ...
            if FromBinContent.Find('-') then begin
                ...
                repeat
                    if UseForReplenishment(FromBinContent) then begin
                        ...
                        QtyAvailToTakeBase -= fnk_QtyBaseFromExistingWhseWkshLines(FromBinContent, WhseWkshTemplateName, WhseWkshName);   //C54°
    ...
    begin
        ...
        TempWhseWkshLine."Due Date" := WorkDate;   //C54°
    ...
    local procedure fnk_IncludeExistingWhseWkstLinesInRemainQtyToReplenish(var par_de_RemainQtyToReplenishBase: Decimal; par_re_ToBinContent: Record "Bin Content"; par_co_WhseWkshTemplateName: Code[10]; par_co_WhseWkshName: Code[10])
    var
        lo_re_WhseWorksheetLine: Record "Whse. Worksheet Line";
    begin
        // *** Bestehende Vorschlagszeilen auch berücksichtigen, sonst werden sie mehrfach angelegt   //C54°
        lo_re_WhseWorksheetLine.SetRange("Worksheet Template Name", par_co_WhseWkshTemplateName);
        lo_re_WhseWorksheetLine.SetRange(Name, par_co_WhseWkshName);
        lo_re_WhseWorksheetLine.SetRange("Location Code", par_re_ToBinContent."Location Code");
        lo_re_WhseWorksheetLine.SetRange("Item No.", par_re_ToBinContent."Item No.");
        lo_re_WhseWorksheetLine.SetRange("To Bin Code", par_re_ToBinContent."Bin Code");
        if lo_re_WhseWorksheetLine.FindSet(false, false) then begin
            repeat
                par_de_RemainQtyToReplenishBase -= lo_re_WhseWorksheetLine."Qty. Outstanding (Base)";
            until lo_re_WhseWorksheetLine.Next = 0;
        end;
    end;

    local procedure fnk_QtyBaseFromExistingWhseWkshLines(par_re_FromBinContent: Record "Bin Content"; par_co_WhseWkshTemplateName: Code[10]; par_co_WhseWkshName: Code[10]) rv_de_ExistingQty: Decimal
    var
        lo_re_WhseWorksheetLine: Record "Whse. Worksheet Line";
    begin
        // *** Bestehende Vorschlagszeilen auch berücksichtigen, sonst werden sie mehrfach angelegt   //C54°
        lo_re_WhseWorksheetLine.SetRange("Worksheet Template Name", par_co_WhseWkshTemplateName);
        lo_re_WhseWorksheetLine.SetRange(Name, par_co_WhseWkshName);
        lo_re_WhseWorksheetLine.SetRange("Location Code", par_re_FromBinContent."Location Code");
        lo_re_WhseWorksheetLine.SetRange("Item No.", par_re_FromBinContent."Item No.");
        lo_re_WhseWorksheetLine.SetRange("From Bin Code", par_re_FromBinContent."Bin Code");
        if lo_re_WhseWorksheetLine.FindSet(false, false) then begin
            repeat
                rv_de_ExistingQty += lo_re_WhseWorksheetLine."Qty. Outstanding (Base)";
            until lo_re_WhseWorksheetLine.Next = 0;
        end;
    end;
