codeunit 50134 INHSalesPost
{
    ...
    // ------------------------------------------------------------
    // Axx°          RBI  25.07.08  Anpassungen in CU übernehmnen
    // B29°          SSC  15.05.18  IF Guiallowed vor Confirm
    // C24°          SSC  19.04.16  Die Funktionalitäten der EP-Rückverfolgung(SalesSetup."Exact Cost Reversing Mandatory") sind erwünscht,
    //                               sollen jedoch nicht verpflichtend sein
    // C27°          SSC  25.10.16  Hooks
    // C54°          RBI  18.03.19  Funktion ShouldPostWhseJnlLine - Bei Lagerorten mit LVS und beim direkten Buchen über VK-Auftrag einen Lagerplatzposten schreiben
    //               RBI  19.12.19  Funktion CheckWarehouse - Bei Lagerorten mit LVS direktes Buchen von Teillieferungen zulassen
    // 
            ...
            if SalesHeader.Invoice then
                if SalesHeader."Document Type" in [SalesHeader."Document Type"::Order, SalesHeader."Document Type"::Invoice] then begin
                    ...
                    if not IsHandled then begin
                        ...
                        cu_SalesMgt.fnk_Cu80_OnBeforeInsertInvoiceLine(SalesHeader, SalesLine, SalesInvHeader, SalesInvLine);   //C27°
    ...
    begin
        ...
        OnAfterPostItemLine(SalesHeader, SalesLine, QtyToInvoice, QtyToInvoiceBase);
    end;
    ...
    begin
        ...
        cu_SalesMgt.fnk_Cu80_OnBeforePostItemJnlLine(SalesHeader, SalesLine);   //C27°
    ...
    begin
        ...
        with SalesLine do begin
            ...
            if (("Document Type" in ["Document Type"::Invoice, "Document Type"::"Credit Memo"]) and
               ...
               or (Location."Directed Put-away and Pick" and (lo_cu_OverallVariableMgt.fnk_GetSalesDirectWhsePosting))  //C54°
            then
                exit(true);
        end;
        exit(false);
    ...
    begin
        ...
        //Axx°:IF NOT IsHandled THEN
        //Axx°:  SalesLine.TESTFIELD("Job No.",'');
        ...
        with TempItemChargeAssgntSales do begin
            ...
            ItemJnlLine2."Description 2" := ItemChargeSalesLine."Description 2";   //Axx°
    ...
    begin
        ...
        //Axx°:IF NOT IsHandled THEN
        //Axx°:  SalesShptLine.TESTFIELD("Job No.",'');
    ...
    begin
        ...
        //Axx°:IF NOT IsHandled THEN
        //Axx°:  ReturnRcptLine.TESTFIELD("Job No.",'');
    ...
    begin
        ...
        with SalesHeader do begin
            ...
            then begin
                //foobaz SSAM TODO: Es kommt manchmal vor dass sich ein großer WA (im laufenden Betrieb) nicht buchen lässt, weil am Ende die Fehlermeldung "Ein anderer User hat..." kommt
                // Workaround, da Ursache bisher nicht ermittelbar: dieses MODIFY für diese Buchung ignorieren mit z.B. vorangestelltem : "IF USERID <> 'KLUK' THEN"
                // Achtung: Auftrag danach neu freigeben und kontrollieren (Möglichkeit, vorab im Debugger bei auftreten des Fehlers schauen wie der Record und die einzelnen Felder ausschauen)
                //IF USERID <> 'SSAM' THEN
                Modify;
                        ...
                        "Document Type"::Order:
                            ...
                            then begin
                                if Location."Directed Put-away and Pick" then
                                    if not lo_cu_OverallVariableMgt.fnk_GetSalesDirectWhsePosting then  //C54°
                                        ShowError := true
                                    else
    ...
    begin
        with GenJnlLine do begin
            ...
            cu_SalesMgt.fnk_Cu80_UpdateGenJnlLineFromSales(GenJnlLine, SalesHeader);   //C27°
            GenJnlPostLine.RunWithCheck(GenJnlLine);
            OnAfterPostCustomerEntry(GenJnlLine, SalesHeader, TotalSalesLine2, TotalSalesLineLCY2, SuppressCommit, GenJnlPostLine);
        end;

        cu_SalesMgt.fnk_Cu80_OnAfterPostCustomerEntry(GenJnlLine, SalesHeader);   //C27°
    end;
    ...
    begin
        ...
        OnBeforeSalesShptLineInsert(
          ...
          ItemLedgShptEntryNo);
        cu_SalesMgt.fnk_Cu80_OnBeforeInsertShipmentLine(SalesShptLine, SalesHeader, SalesShptHeader, SalesLine, CostBaseAmount, TempServiceItem2, TempServiceItemComp2);   //C27°
        ...
        with TempSalesLine do begin
            ...
            if FindSet then
                repeat
                    ...
                    //START Axx° ---------------------------------
                    if WhseHandlingRequired(TempSalesLine) and (TempSalesLine.Quantity <> 0) then begin
                        TempSalesLine.Lieferstatus := cu_SalesMgt.FNK_WhseAuftragsStatusPrüfen(TempSalesLine);
                    end;
                    TempSalesLine."Zu liefern (manuell)" := false;
                    //STOP  Axx° ---------------------------------
        ...
        with TempSalesLine do begin
            ...
            if FindSet then
                repeat
                    ...
                    //  SalesOrderLine."Document Type"::Order,
                    //  SalesShptLine."Order No.",SalesShptLine."Order Line No.");
                    if SalesOrderLine.Get(SalesOrderLine."Document Type"::Order, SalesShptLine."Order No.", SalesShptLine."Order Line No.")
                    then begin
                        //STOP  Axx° ---------------------------------
    ...
    [IntegrationEvent(false, false)]
    local procedure OnAfterPostItemLine(SalesHeader: Record "Sales Header"; SalesLine: Record "Sales Line"; QtyToInvoice: Decimal; QtyToInvoiceBase: Decimal)
    begin
    end;
