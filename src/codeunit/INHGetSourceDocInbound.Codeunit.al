codeunit 50115 INHGetSourceDocInbound
{
    // +---------------------------------------------+
    // +                                             +
    // +           Inhaus Handels GmbH               +
    ...
    // C54°          SSC  28.02.20  Felder im WE-Kopf vorbefüllen
    ...
    var
        ...
        lo_re_WhseRqst2: Record "Warehouse Request";
    begin
        ...
        //START C54° ---------------------------------
        if WhseReceiptHeader.Get(WhseReceiptHeader."No.") then begin
            if WhseReceiptHeader."Assigned User ID" = '' then begin
                WhseReceiptHeader.Validate("Assigned User ID", UserId);
            end;
            if lo_re_WhseRqst2."Source Document" = lo_re_WhseRqst2."Source Document"::"Sales Return Order" then begin
                if WhseReceiptHeader."Vendor Shipment No." = '' then begin
                    WhseReceiptHeader.Validate("Vendor Shipment No.", lo_re_WhseRqst2."Source No.");
                end;
            end;
