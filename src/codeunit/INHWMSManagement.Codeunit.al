codeunit 50165 INHWMSManagement
{
    // +---------------------------------------------+
    // +                                             +
    // +           Inhaus Handels GmbH               +
    ...
    // ------------------------------------------------------------
    // C54°          RBI  18.03.19  Funktion CreateWhseJnlLine - Bei Lagerorten mit LVS beim direkten Buchen über VK-Auftrag einen Lagerplatzposten schreiben
    //               RBI  27.11.19  Funktion CreateWhseJnlLine - Bei Lagerorten mit LVS beim Buchen über das Artikel Buch.-Blatt Lagerplatzposten schreiben
    //               RBI  11.12.19  Funktion CreateWhseJnlLine - Bei Lagerorten mit LVS beim direkten Buchen über EK- und VK-Reklamation einen Lagerplatzposten auf den LP von der EK/VK-Zeile schreiben.
    //                              Ansonsten wird ein Lageplatzposten für den Ausgleichslagerplatz erzeugt.
    //               SSC  18.06.20  IC-Aufträge aufrufen ermöglichen, fnk_ShowWhseDocCard
    //                              RUN verwenden nicht RUNMODAL
    //               SSC  22.07.20  Gewicht von Karte verwenden wenn bei der Einheit nicht gepflegt
    ...
    begin
        if ItemNo <> '' then begin
            ...
            if ItemUnitOfMeasure.Weight = 0 then begin
                GetItem(ItemNo);
                Weight := Qty * Item."Net Weight" * ItemUnitOfMeasure."Qty. per Unit of Measure";
            end;
            //STOP  C54° ---------------------------------
        end else begin
            Cubage := 0;
        ...
        case SourceType of
            DATABASE::"Sales Line":
                begin
                    ...
                    if SalesHeader.Get(SourceSubType, SourceNo) then begin
                        ...
                        lo_cu_SalesMgt.fnk_ShowOrder(SourceNo);
                        //START C54° ---------------------------------
                    end;
                end;
            DATABASE::"Purchase Line":
    ...
    begin
        with ItemJnlLine do
            ...
            then begin
                ...
                if Location."Directed Put-away and Pick" then
                    if ("Entry Type" in ["Entry Type"::"Assembly Output", "Entry Type"::"Assembly Consumption"])
                      or (lo_cu_OverallVariableMgt.fnk_GetSalesDirectWhsePosting) or (lo_cu_OverallVariableMgt.fnk_GetDirectWhsePosting) or (lo_cu_OverallVariableMgt.fnk_GetPurchaseDirectWhsePosting)  //C54°
                    then
    ...
    [Scope('Internal')]
    procedure fnk_ShowWhseDocCard(WhseDocType: Option Receipt,"Posted Receipt",Shipment,"Internal Put-away","Internal Pick",Production,,Assembly; WhseDocNo: Code[20])
    var
        ...
        WhseInternalPickHdr: Record "Whse. Internal Pick Header";
        WhseInternalPutawayHdr: Record "Whse. Internal Put-away Header";
        ProdOrderHdr: Record "Production Order";
        AssemblyHdr: Record "Assembly Header";
    begin
        // *** Angepasste Kopie von ShowWhseDocLine für die Karten   //C54°
        case WhseDocType of
            WhseDocType::Receipt:
                begin
                    WhseRcptHdr.SetRange("No.", WhseDocNo);
                    PAGE.Run(PAGE::"Warehouse Receipt", WhseRcptHdr);
                end;
            WhseDocType::"Posted Receipt":
                begin
                    PostedWhseRcptHdr.SetRange("No.", WhseDocNo);
                    PAGE.Run(PAGE::"Posted Whse. Receipt", PostedWhseRcptHdr);
                end;
            WhseDocType::Shipment:
                begin
                    WhseShptHdr.SetRange("No.", WhseDocNo);
                    PAGE.Run(PAGE::"Warehouse Shipment", WhseShptHdr);
                end;
            WhseDocType::"Internal Put-away":
                begin
                    WhseInternalPutawayHdr.SetRange("No.", WhseDocNo);
                    PAGE.Run(PAGE::"Whse. Internal Put-away", WhseInternalPutawayHdr);
                end;
            WhseDocType::"Internal Pick":
                begin
                    WhseInternalPickHdr.SetRange("No.", WhseDocNo);
                    PAGE.Run(PAGE::"Whse. Internal Pick", WhseInternalPickHdr);
                end;
            WhseDocType::Production:
                begin
                    ProdOrderHdr.SetRange(Status, ProdOrderHdr.Status::Released);
                    ProdOrderHdr.SetRange("No.", WhseDocNo);
                    PAGE.Run(PAGE::"Production Order List", ProdOrderHdr);
                end;
            WhseDocType::Assembly:
                begin
                    AssemblyHdr.SetRange("Document Type", AssemblyHdr."Document Type"::Order);
                    AssemblyHdr.SetRange("No.", WhseDocNo);
