codeunit 50176 INHCreatePick
{
    ...
    // C54°          RBI  31.05.19  Funktion CreateTempActivityLine - Feld Priority für Kommissionierung füllen
    //                              Funktion CreateNewWhseDoc - Bei Überschreitung von Max. Volumen pro Kommissfahrzeug neuen Kommissbeleg erstellen
    //                              Neue Funktion fnk_CalculateWhseActivityCubage - Berechnet das Volumen des Kommissbeleg
    //                              Neue Funktion fnk_SetMaxCubagePickVehicle - Setzt das Max. Volumen pro Kommissfahrzeug
    //               RBI  18.07.19  Funktion CreateTempActivityLine - Beim Erstellen einer Kommiss zu einer Kommisszeile muss eine hohe Prio gesetzt und die Zone und der Lagerplatz gelöscht werden
    //               RBI  12.09.19  Funktion BinContentExists - Bei Kommiss Erstellung aus Kommissvorschlag Fixed-Bins mit Vorrang behandeln
    //               RBI  04.12.19  Funktion CreateTempActivityLine - Feld "Interim Bin Code" für Lagerplatzumlagerung füllen
    //               RBI  27.02.20  Funktion fnk_SetTempBinContent - Schreibt in temporäre Tabelle die gewünschte Sortierung.
    //                              Dazu in Funktion BinContentExists,FindPickBin,FindBreakBulkBin,FindSmallerUOMBin die Tabelle "Bin Content" auf temporär umgestellt.
    ...
    var
        ...
        "+++VAR_INHAUS+++": Integer;
        de_MaxCubagePickVehicle: Decimal;
        de_MaxWeightPickVehicle: Decimal;
    ...
    local procedure BinContentExists(var BinContent: Record "Bin Content" temporary; ItemNo: Code[20]; LocationCode: Code[10]; UOMCode: Code[10]; VariantCode: Code[10]; CrossDock: Boolean; LNRequired: Boolean; SNRequired: Boolean): Boolean
    var
        lo_re_TempBinContent: Record "Bin Content" temporary;
    begin
        //C54°: Parameter BinContent auf TEMPORARY gesetzt
        //START C54° ---------------------------------
        if fnk_SetTempBinContent(BinContent, ItemNo, LocationCode, UOMCode, VariantCode, WhseItemTrkgExists) then begin
            BinContent.SetRange("Cross-Dock Bin", CrossDock);
            BinContent.SetRange("Unit of Measure Code", UOMCode);
            exit(BinContent.FindSet);
        end else begin
            //STOP  C54° ---------------------------------
            with BinContent do begin
                ...
                if WhseSource = WhseSource::"Pick Worksheet" then   //C54°
                    SetCurrentKey("Location Code", "Item No.", "Variant Code", "Cross-Dock Bin", "Qty. per Unit of Measure", Fixed, "Bin Ranking");   //C54°
                if WhseItemTrkgExists then begin
                    if LNRequired then
                        SetRange("Lot No. Filter", TempWhseItemTrackingLine."Lot No.")
                    else
    ...
    var
        FromBinContent: Record "Bin Content" temporary;
        FromQtyToPick: Decimal;
        FromQtyToPickBase: Decimal;
        ToQtyToPick: Decimal;
        ToQtyToPickBase: Decimal;
    ...
    begin
        //C54°: Variable FromBinContent auf TEMPORARY gesetzt
        // Directed put-away and pick
        GetBin(LocationCode, ToBinCode);
        GetLocation(LocationCode);
        with FromBinContent do
            if BinContentExists(
                 FromBinContent, ItemNo, LocationCode, UnitofMeasureCode, VariantCode, CrossDock, LNRequired, SNRequired)
            then begin
    ...
    var
        ...
        IsHandled: Boolean;
        "+++VAR_Inhaus+++": Integer;
        lo_cu_LogisticMobileMgt: Codeunit LogisticMobileMgt;
        lo_bo_CreateOnlyOnePick: Boolean;
    begin
    ...
    var
        "+++VAR_Inhaus+++": Integer;
        lo_de_WhseActivityCubage: Decimal;
        lo_de_WhseActivityWeight: Decimal;
    begin
        ...
        then begin
            ...
            if (MaxNoOfLines > 0) and (NoOfLines > MaxNoOfLines) then
                CreateWhseActivHeader(
                  ...
                  NoOfSourceDoc, NoOfLines, WhseDocCreated);
            //START C54° ---------------------------------
            if (de_MaxCubagePickVehicle > 0) or (de_MaxWeightPickVehicle > 0) then begin
                fnk_CalculateWhseActivityCubageAndWeight(lo_de_WhseActivityCubage, lo_de_WhseActivityWeight);
                if (lo_de_WhseActivityCubage > de_MaxCubagePickVehicle) or (lo_de_WhseActivityWeight > de_MaxWeightPickVehicle) then begin
                    CreateWhseActivHeader(
                      TempWhseActivLine."Location Code", FirstWhseDocNo, LastWhseDocNo,
                      NoOfSourceDoc, NoOfLines, WhseDocCreated);
                end;
    ...
    begin
        ...
        //START A08° ---------------------------------
        WhseActivHeader."Gehört zu WA" := TempWhseActivLine."Gehört zu WA";
        WhseActivHeader.IC_Company := TempWhseActivLine.IC_Company;
        //STOP  A08° ---------------------------------
    ...
    begin
        ...
        TempWhseItemTrackingLineBuffer.SetSourceFilter2(SourceBatchName, SourceProdOrderLine);
    ...
    local procedure CalcQtyCanBePicked(LocationCode: Code[10]; ItemNo: Code[20]; VariantCode: Code[10]; LotNo: Code[50]; SerialNo: Code[50]; IsMovement: Boolean): Decimal
    var
    ...
    var
        WhseSource2: Option;
        "+++VAR_Inhaus+++": Integer;
        lo_re_INHAUSEinrichtung: Record "INHAUS Einrichtung";
    ...
    begin
        ...
        with TempWhseActivLine do begin
            ...
            //START A08° ---------------------------------
            IC_Company := WhseShptLine.IC_Company;
            Artikeltyp := WhseShptLine.Artikeltyp;
            "Gehört zu WA" := WhseShptLine."No.";
            //STOP  A08° ---------------------------------

            //START C54° ---------------------------------
            if lo_cu_OverallVariableMgt.fnk_GetCreatePickOutofPick then begin
                lo_re_INHAUSEinrichtung.Get;
                Priority := lo_re_INHAUSEinrichtung."Single Picking Priority";
                if "Action Type" = "Action Type"::Take then begin
                    "Zone Code" := '';
                    "Bin Code" := '';
                end;
                lo_cu_OverallVariableMgt.fnk_ClearCreatePickOutofPick;
            end else
                Priority := WhseWkshLine.Priority;
            "Interim Bin Code" := WhseWkshLine."Interim Bin Code";
            //STOP  C54° ---------------------------------
    ...
    local procedure SumWhseEntries(ItemNo: Code[20]; LocationCode: Code[10]; VariantCode: Code[10]; IsLNRequired: Boolean; LotNo: Code[50]; IsSNRequired: Boolean; SerialNo: Code[50]; BinTypeCodeFilter: Text; BinCodeFilter: Text; ExcludeDedicatedBins: Boolean): Decimal
    var
        WhseEntry: Record "Warehouse Entry";
    begin
        with WhseEntry do begin
    ...
    begin
        with AsmLine do begin
            ...
            if lo_cu_ItemMgt.fnk_UseAssemblyWithSetItem then begin
                QtyToPickBase := WhseShptLine."Qty. (Base)" * AsmLine."Quantity per";
                QtyToPick := WhseShptLine.Quantity * AsmLine."Quantity per";
            end else begin
                //STOP  C39° ---------------------------------
    ...
    local procedure fnk_CalculateWhseActivityCubageAndWeight(par_de_WhseActivityCubage: Decimal; par_de_WhseActivityWeight: Decimal)
    var
        lo_re_WarehouseActivityLine: Record "Warehouse Activity Line";
        lo_cu_WMSManagement: Codeunit "WMS Management";
        lo_de_Cubage: Decimal;
        lo_de_Weight: Decimal;
    begin
        //C54°
        //Bisherige Zeilen in der Kommissionierung
        lo_re_WarehouseActivityLine.SetRange("Activity Type", WhseActivHeader.Type);
        lo_re_WarehouseActivityLine.SetRange("No.", WhseActivHeader."No.");
        lo_re_WarehouseActivityLine.SetRange("Action Type", lo_re_WarehouseActivityLine."Action Type"::Take);
        if lo_re_WarehouseActivityLine.FindSet then begin
            repeat
                par_de_WhseActivityCubage += lo_re_WarehouseActivityLine.Cubage;
                par_de_WhseActivityWeight += lo_re_WarehouseActivityLine.Weight;
            until lo_re_WarehouseActivityLine.Next = 0;
        end;
        //Die nächste hinzukommende Zeile
        if Location."Directed Put-away and Pick" then begin
            lo_cu_WMSManagement.CalcCubageAndWeight(
              TempWhseActivLine."Item No.", TempWhseActivLine."Unit of Measure Code", TempWhseActivLine."Qty. to Handle", lo_de_Cubage, lo_de_Weight);
            par_de_WhseActivityCubage += lo_de_Cubage;
            par_de_WhseActivityWeight += lo_de_Weight;
        end;
    end;

    [Scope('Internal')]
    procedure fnk_SetMaxCubageAndWeightPickVehicle(par_de_MaxCubagePickVehicle: Decimal; par_de_MaxWeightPickVehicle: Decimal)
    begin
        //C54°
        de_MaxCubagePickVehicle := par_de_MaxCubagePickVehicle;
        de_MaxWeightPickVehicle := par_de_MaxWeightPickVehicle;
    end;

    local procedure fnk_SetTempBinContent(var par_re_TempBinContent: Record "Bin Content" temporary; par_co_ItemNo: Code[20]; par_co_LocationCode: Code[10]; par_co_UOMCode: Code[10]; par_co_VariantCode: Code[10]; par_bo_WhseItemTrkgExists: Boolean) rv_bo_FoundBin: Boolean
    var
        lo_re_BinBuffer: Record Bin temporary;
    ...
    begin
        //C54°
        //Bei Änderungen bitte auch Cu50051.fnk_UpdateBinPickWksh und Cu50051.fnk_Cu7311_OnAfterTransferFromWhseShptLine beachten
        //WhseWkshLine - Globale Variable die an dieser Stelle gefüllt ist. Kann für das Holen des Herkunftsbeleg verwendet werden.

        with lo_re_BinContent do begin
            //Füllen der temporären Tabelle
            SetRange("Location Code", par_co_LocationCode);
            SetRange("Item No.", par_co_ItemNo);
            SetRange("Variant Code", par_co_VariantCode);
            Ascending(false);
            if FindSet then begin
                repeat
                    par_re_TempBinContent := lo_re_BinContent;
                    if par_re_TempBinContent.Insert then;
                until lo_re_BinContent.Next = 0;
            end;

            //Bei Artikeln mit Charge/Seriennr. passt die Standardlogik, dass füllen der temporären Tabelle muss jedoch trotzdem vorher passieren,
            //  da mit dieser weitergearbeitet wird
            if par_bo_WhseItemTrkgExists then
                exit(false);

            if not lo_re_Item.Get(par_co_ItemNo) then
                exit(false);

            if CalledFromWksh and (WhseWkshLine."Whse. Document Type" = WhseWkshLine."Whse. Document Type"::Shipment) then begin
                lo_re_WhseShptLine.Get(WhseWkshLine."Whse. Document No.", WhseWkshLine."Whse. Document Line No.");
            end else begin
                lo_re_WhseShptLine := WhseShptLine;
            end;
            if lo_re_WhseShptLine."No." <> '' then begin
                lo_cu_LogisticsMgt.fnk_FindCorrectBin4Pick(lo_re_WhseShptLine, '', false, lo_re_BinBuffer);
                if lo_re_BinBuffer.FindSet(false, false) then begin
                    repeat
                        lo_co_Bin := lo_re_BinBuffer.Code;
                        if lo_co_Bin <> '' then begin
                            if par_re_TempBinContent.Get(par_co_LocationCode, lo_co_Bin, par_co_ItemNo, par_co_VariantCode, par_co_UOMCode) then begin
                                rv_bo_FoundBin := true;
                                if lo_re_BinBuffer."Bin Ranking" <> 0 then begin
                                    par_re_TempBinContent.Sorting := lo_re_BinBuffer."Bin Ranking";
                                end else begin
                                    i += 1;
                                    par_re_TempBinContent.Sorting := i;
                                end;
                                par_re_TempBinContent.Modify;
                            end;
                        end;
                    until lo_re_BinBuffer.Next = 0;
                end;
            end;

            par_re_TempBinContent.SetCurrentKey(Sorting);
            par_re_TempBinContent.Ascending(false);
        end;
    end;
}
