codeunit 50120 INHPostPurchDelete
{
    // +---------------------------------------------+
    // +                                             +
    // +           Inhaus Handels GmbH               +
    ...
    // ------------------------------------------------------------
    // B19°.1        SSC  22.11.13  neue fnk_SetInvoiceAction (war früher in Cu90)
    ...
    begin
        with PurchHeader do begin
            ...
            //TODO evtl.: Setup für Codes machen
            if op_DocAction = op_DocAction::Post then begin
                SourceCode.Get('FIBUBEGEB');
            end else
                if op_DocAction = op_DocAction::Correction then begin
                    SourceCode.Get('FIBUBESTOR');
                end else begin
                    SourceCode.Get(SourceCodeSetup."Deleted Document");
                end;
            //STOP  B19°.1 ---------------------------------
    ...
    [Scope('Internal')]
    procedure fnk_SetDocAction(par_op_DocAction: Option " ",Post,Correction)
    begin
        // *** Es gibt mehrere Aktionen warum eine EK-Rechnung/Gutschrift gelöscht wird   //B19°.1
        //     meist ist es, in der Realität, eigentlich keine wirkliche Löschung
        //     Z.b.: EK-Rechnung wird bei Rechnungseingang vorerfasst; handelt es sich um eine Fibu-Rechnung(kein Materialfluss)
        //           wird im Fibu-Buchblatt gebucht, und der EK-Beleg im Zuge dessen gelöscht
        op_DocAction := par_op_DocAction;
    end;
