codeunit 50131 INHReminderMake
{
    // +---------------------------------------------+
    // +                                             +
    // +           Inhaus Handels GmbH               +
    ...
    // ------------------------------------------------------------
    // A57°.2        SSC  28.06.12  Endtexte nach den "Nicht fälligen offenen Posten" andrucken
    // A57°.3        SSC  05.02.20  Zahlungsposten sollen die Mahnstufe auch nicht erhöhen
    //               SSC  10.06.20  Anpassung abgeändert; Betrag muss positiv sein damit Mahnstufe erhöht wird
    ...
    begin
        with Cust do begin
            ...
            if MakeDoc and (CustAmount > 0) and (CustAmountLCY(CurrencyCode, CustAmount) >= ReminderTerms."Minimum Amount (LCY)") then begin
                ...
                ReminderHeader.Modify;
                ReminderHeader.InsertEndTexts(ReminderHeader);   //A57°.2
            end;
        end;
                    ...
                    if CustLedgEntry."On Hold" = '' then begin
                        ...
                        if MarkEntry then begin
                            ...
                            if (ReminderLevel."No." > MaxReminderLevel) and
                               ...
                               //(CustLedgEntry."Document Type" <> CustLedgEntry."Document Type"::Payment) AND //A57°.3
                               (CustLedgEntry."Remaining Amount" > 0) and  //A57°.3
                               IsGracePeriodExpired
                            then
