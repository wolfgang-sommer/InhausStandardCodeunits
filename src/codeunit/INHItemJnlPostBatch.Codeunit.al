codeunit 50119 INHItemJnlPostBatch
{
    // +---------------------------------------------+
    // +                                             +
    // +           Inhaus Handels GmbH               +
    ...
    // ------------------------------------------------------------
    // A28°          RBI  25.07.08  Inventur
    // B62°          SSC  05.09.16  Verzögertes Updateinterval
    // C54°          RBI  23.01.20  PostLines - Funktion auf Global umgestellt - wird für das IC-Geschäft benötigt
    ...
    begin
        ...
        with ItemJnlLine do begin
            ...
            if ItemJnlTemplate."Increment Batch Name" then
                if not ItemJnlLine3.FindLast and not ("Journal Template Name" = 'INVENTUR') then begin   //A28°
    ...
    [Scope('Internal')]
    procedure fnk_OpenWindow(par_co_JournalBatchName: Code[10])
    begin
        //C54°
        NoOfRecords := 1;
        Window.Open(
          ...
          Text002 +
          Text005);
        Window.Update(1, par_co_JournalBatchName);
    end;
}
