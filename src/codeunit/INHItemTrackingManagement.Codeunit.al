codeunit 50124 INHItemTrackingManagement
{
    // +---------------------------------------------+
    // +                                             +
    // +           Inhaus Handels GmbH               +
    ...
    // ------------------------------------------------------------
    // Axx°          RBI  29.07.08  Anpassungen übernommen
    // Axx°.1        SSC  29.10.10  Bugfix bei BVO mit Artikel incl. Seriennummer
    // Axx°.2        SSC  21.01.16  Andere InfoForm
    // C43°.1        SSC  12.07.18  Aktuellen Lagerort vom Beleg für Reservierungsposten verwenden

    Permissions = TableData "Tracking Specification" = rd,
    ...
    var
        ...
        "+++VAR_INHAUS+++": Boolean;
        bo_UseDefLookupLotSNoInfo: Boolean;
    ...
    begin
        ...
        if ItemJnlLine."Reason Code" <> 'XMLDX' then
            ReservEntry.SetRange("Source Type", DATABASE::"Item Journal Line")
        else
        ...
        //START Axx°.1 ---------------------------------
        // Bei BVO GmbH nach AG wird Ursachencode XMLDX gefüllt, die obrigen Anpassung A24°(wo nötig? evtl. veraltet?) verhindert,
        // dass bei einem Artikel mit Seriennummer diese gefunden wird
        if (ItemJnlLine."Reason Code" = 'XMLDX') and (ReservEntry.IsEmpty) then
            ReservEntry.SetRange("Source Type", DATABASE::"Item Journal Line");
        //STOP  Axx°.1 ---------------------------------
    ...
    procedure CopyHandledItemTrkgToInvLine2(FromPurchLine: Record "Purchase Line"; ToPurchLine: Record "Purchase Line")
    begin
        ...
        with TempItemLedgEntryBuf do
            if FindSet then begin
                ...
                repeat
                    ...
                    ReservEntry."Location Code" := ToSalesLine."Location Code";
                    //STOP  C43°.1 ---------------------------------
    ...
    procedure WhseActivitySignFactor(WhseActivityLine: Record "Warehouse Activity Line"): Integer
    begin
    ...
    [Scope('Internal')]
    procedure fnk_SetUseDefLookupLotSNoInfo(par_bo_UseDefLookupLotSNoInfo: Boolean)
    begin
        //Axx°.2
        bo_UseDefLookupLotSNoInfo := par_bo_UseDefLookupLotSNoInfo;
    end;
}
