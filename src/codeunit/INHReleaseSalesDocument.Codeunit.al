codeunit 50130 INHReleaseSalesDocument
{
    // +---------------------------------------------+
    // +                                             +
    // +           Inhaus Handels GmbH               +
    ...
    // ------------------------------------------------------------
    // B43°.1        SSC  08.05.12  Kreditlimit-Prüfung bei Freigabe (war in Fo42)
    // B43°.2        SSC  01.10.15  Kreditlimit-Prüfung auf Unternehmensgruppe, in Meldung berücksichtigen (war in Fo42)
    // B81°          SSC  07.10.13  Änderungsportokoll bei Statuswechsel
    //               SSC  14.03.14  Neue fnk_PerformManualReleaseUser, fnk_PerformManualReopenUser für Aufruf aus der Form;
    //                              Code dorhin verschoben. Nur protokollieren wenn über die Form Status geändert wird; Logistiker, NAS nicht sinnvoll.
    //               SSC  07.06.18  Wegen Performance nicht ins Änderungsprotokoll schreiben sondern ins Feld "letzte Änderung von"
    // B90°          SSC  27.11.19  SalesHeader Info mitgeben dass Logistik Buchung um das später abzugreifen
    // C10°          SSC  20.05.16  Angebote auch freigeben wenn nur eine Kontaktnr. hinterlegt ist
    ...
    begin
        ...
        SalesHeader.fnk_SetIsLogisticProcess(Rec.fnk_GetIsLogisticProcess);   //B90°
        Code;
        Rec := SalesHeader;
    end;
    ...
    begin
        //TODO: Funktionen werden in Pa42 verwendet -> nach Cu50023 verschieben
    end;

    [Scope('Internal')]
    procedure fnk_PerformManualReleaseUser(var SalesHeader: Record "Sales Header")
    var
        lo_re_Cust: Record Customer;
        lo_re_xSalesHdr: Record "Sales Header";
        lo_pa_CustCheckCreditLimit: Page "Check Credit Limit";
        lo_cu_ChangeLogMgt: Codeunit "Change Log Management";
        lo_cu_SalesMgt: Codeunit SalesMgt;
        LoTextCreditLimitExceeded: Label 'Das Kreditlimit dieses Debitors(%1) wurde überschritten. Bitte melden Sie sich in der Buchhaltung!\Freigabe nicht möglich.';
        lo_cu_ServiceProcessMgt: Codeunit "Sales to Service Mgt.";
    begin
        // *** Extra Funktion für Aufruf aus der Form   //B81°

        with SalesHeader do begin
            lo_re_xSalesHdr := SalesHeader;

            //START B43°.1 ---------------------------------
            if "Document Type" = "Document Type"::Order then begin
                if not "Skip CreditLimitCheck" and (Status <> Status::Released) then begin
                    if lo_pa_CustCheckCreditLimit.SalesHeaderShowWarning(SalesHeader) then begin
                        "Kreditlimit überschritten" := true;
                        Modify(false);
                        lo_re_Cust.Get("Bill-to Customer No.");
                        if lo_re_Cust.fnk_UsesCompGrpCreditLimit then begin
                            Message(LoTextCreditLimitExceeded, lo_re_Cust.FieldCaption("Company Group") + ' ' + lo_re_Cust."Company Group");
                        end else begin
                            Message(LoTextCreditLimitExceeded, "Bill-to Customer No.");
                        end;
                        exit;
                    end;
                end;
            ...
            //STOP  B43°.1 ---------------------------------
            lo_cu_SalesMgt.fnk_ConfirmShipToAddr(SalesHeader);   //Axx°.7

            PerformManualRelease(SalesHeader);

            //lo_cu_ChangeLogMgt.fnk_InsertLogEntryT36(SalesHeader,lo_re_xSalesHdr,FIELDNO(Status),1);
            "letzte Änderung von" := UserId;

            //START B43°.1 ---------------------------------
            "Skip CreditLimitCheck" := false;
            Modify(false);
            //STOP  B43°.1 ---------------------------------
    ...
    [Scope('Internal')]
    procedure fnk_PerformManualReopenUser(var SalesHeader: Record "Sales Header")
    var
        lo_re_xSalesHdr: Record "Sales Header";
        lo_cu_ChangeLogMgt: Codeunit "Change Log Management";
    begin
        ...
        //lo_cu_ChangeLogMgt.fnk_InsertLogEntryT36(SalesHeader,lo_re_xSalesHdr,SalesHeader.FIELDNO(Status),1);
        SalesHeader."letzte Änderung von" := UserId;
    end;
}
