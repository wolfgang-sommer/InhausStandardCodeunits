codeunit 50151 INHWhseActivityRegister
{
    // +---------------------------------------------+
    // +                                             +
    // +           Inhaus Handels GmbH               +
    ...
    // C27°          SSC  22.08.17  Hooks
    // C54°          SSC  21.11.19  Logistik; IC-Aufträge berücksichtigen
    // C84°          SSC  05.08.22  User vom Autostore auch in die Posten übertragen
    // 
    // UPGBC140 2022-05-13 1CF_DAL Upgrade to BC140
    //          New local variable WhseShptLine - Code()
    //          New code - Code()
    ...
    begin
        ...
        lo_cu_LogisticsMgt.fnk_Cu7307_OnBeforeCode(WhseActivLine);   //C27°
        ...
        with WhseActivHeader do begin
            ...
            CalcFields("Source Document", "Source No.");   //A08°
        ...
        //>> UPGBC140_DAL Start
        if WhseShptLine.Get(WhseActivLine."Whse. Document No.", WhseActivLine."Whse. Document Line No.") then;
        //<< UPGBC140_DAL End
        lo_cu_LogisticsMgt.fnk_Cu7307_OnAfterCode(WhseActivHeader, WhseActivLine, WhseShptLine);   //C27°
    ...
    begin
        with WhseActivLine do begin
            ...
            //START C84° ---------------------------------
            if WhseActivLine."MDE User" <> '' then begin
                WhseJnlLine."User ID" := WhseActivLine."MDE User";
            end else
                //STOP  C84° ---------------------------------
    ...
    begin
        ...
        //START C54° ---------------------------------
        if WhseShptLine."Completely Picked" then begin
            WhseShptLine."Completely Picked All But" := true;
        end;
        //STOP  C54° ---------------------------------
    ...
    begin
        if not GuiAllowed then HideDialog := true;   //A08°
        if not HideDialog then
            case ControlNo of
                1:
                    begin
    ...
    begin
        ...
        if TempWhseActivLineToReserve.FindSet then
            repeat
                ItemTrackingMgt.CheckWhseItemTrkgSetup(TempWhseActivLineToReserve."Item No.", WhseSNRequired, WhseLNRequired, false);
                if ((TempWhseActivLineToReserve."Serial No." <> '') = WhseSNRequired) and
                   ((TempWhseActivLineToReserve."Lot No." <> '') = WhseLNRequired)
                then begin
                    //START C54° ---------------------------------
                    //SalesLine.GET(
    ...
    [IntegrationEvent(false, false)]
    local procedure OnUpdateWhseShptLineOnBeforeWhseShptLineModify(var WarehouseShipmentLine: Record "Warehouse Shipment Line"; WarehouseActivityLine: Record "Warehouse Activity Line")
    begin
    end;
}
