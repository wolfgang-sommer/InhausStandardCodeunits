codeunit 50155 INHWhseJnlRegisterLine
{
    // +---------------------------------------------+
    // +                                             +
    // +           Inhaus Handels GmbH               +
    ...
    // ------------------------------------------------------------
    // Axx°          SSC  09.07.18  Permissions ergänzt für Table 7354-Bin
    // C54°          RBI  21.03.19  DeleteFromBinContent - Bei bestimmten Artikel Minusbuchung erlauben
    //               RBI  14.11.19  InitWhseEntry - MDE User in den Lagerplatzposten schreiben
    //               RBI  11.12.19  Event OnAfterInsertToBinContent
    //               SSC  18.12.19  Event OnAfterRegisterWhseJnlLine
    //               SSC  05.10.20  Event OnBeforeInsertToBinContent
    ...
    begin
        ...
        OnAfterRegisterWhseJnlLine(WhseJnlLine, GlobalWhseEntry);   //C54°
    end;
    ...
    var
        ...
        WMSMgt: Codeunit "WMS Management";
        "+++VAR_Inhaus+++": Integer;
        lo_cu_LogisticsMgt: Codeunit LogisticMobileMgt;
    begin
        ...
        with WhseJnlLine do begin
            ...
            //START C54° ---------------------------------
            if lo_cu_LogisticsMgt.fnk_GetActiveMDEUser <> '' then
                WhseEntry."User ID" := lo_cu_LogisticsMgt.fnk_GetActiveMDEUser
            else
                //STOP  C54° ---------------------------------
                ...
                if FromBinContent.Quantity + Quantity = 0 then begin
                    ...
                    if FromBinContent."Quantity (Base)" + "Qty. (Base)" < 0 then begin
                        ...
                        if not IsHandled then begin
                            //START C54° ---------------------------------
                            if lo_cu_ItemMgt.fnk_NegativeInvAllwd("Item No.") then
                                exit;
                            //STOP  C54° ---------------------------------
    ...
    begin
        OnBeforeInsertToBinContent(WhseEntry);   //C54°
        ...
        OnAfterInsertToBinContent(WhseEntry, BinContent);   //C54°
    end;
    ...
    [BusinessEvent(false)]
    local procedure OnBeforeInsertToBinContent(var par_re_WhseEntry: Record "Warehouse Entry")
    begin
    end;

    [BusinessEvent(false)]
    local procedure OnAfterInsertToBinContent(var par_re_WhseEntry: Record "Warehouse Entry"; var par_re_BinContent: Record "Bin Content")
    begin
    end;

    [BusinessEvent(false)]
    local procedure OnAfterRegisterWhseJnlLine(var WhseJnlLine: Record "Warehouse Journal Line"; var GlobalWhseEntry: Record "Warehouse Entry")
    begin
    end;
}
