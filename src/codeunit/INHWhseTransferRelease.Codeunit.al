codeunit 50161 INHWhseTransferRelease
{
    // +---------------------------------------------+
    // +                                             +
    // +           Inhaus Handels GmbH               +
    ...
    var
        ...
        lo_re_TransLine: Record "Transfer Line";
    begin
        with WarehouseRequest do begin
            ...
            //START A08° ---------------------------------
            //Lieferdatum bestimmen (das kleinste Lieferdatum aus den Zeilen)
            lo_re_TransLine.SetCurrentKey("Document No.", "Shipment Date");
            lo_re_TransLine.SetRange("Document No.", TransferHeader."No.");
            lo_re_TransLine.SetFilter("Qty. to Ship", '<>0');
            lo_re_TransLine.SetFilter("Derived From Line No.", '0');
            lo_re_TransLine.SetRange(Lieferstatus, lo_re_TransLine.Lieferstatus::"Lieferfähig");
            if lo_re_TransLine.FindFirst then begin
                Lieferdatum := lo_re_TransLine."Shipment Date";
                Auftragsstatus := TransferHeader.Lieferstatus;
            end else begin
                Lieferdatum := TransferHeader."Shipment Date";
                Auftragsstatus := TransferHeader.Lieferstatus;
            end;
            //STOP  A08° ---------------------------------
        end;
