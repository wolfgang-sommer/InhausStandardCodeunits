codeunit 50147 INHUndoPurchaseReceiptLine
{
    // +---------------------------------------------+
    // +                                             +
    // +           Inhaus Handels GmbH               +
    ...
    // ------------------------------------------------------------
    // Axx°          RBI  29.07.08  Anpassungen in CU übernommen.
    // C23°          SSC  02.03.16  IC-Entkoppelt
    // C24°          SSC  11.11.16  Storno zum Workdate
    // C27°          SSC  10.01.20  OnBeforeUndoPurchRcptLine
    //               SSC  04.02.20  OnAfterUndoPurchRcptLine
    ...
    begin
        ...
        if not GuiAllowed then SetHideDialog(true);   //C23°
    ...
    begin
        OnBeforeUndoPurchRcptLine(PurchRcptLine);   //C27°
        ...
        OnAfterUndoPurchRcptLine(PurchRcptLine);   //C27°
    end;
    ...
    [IntegrationEvent(false, false)]
    local procedure OnBeforeUndoPurchRcptLine(var PurchRcptLine: Record "Purch. Rcpt. Line")
    begin
    end;

    [IntegrationEvent(false, false)]
    local procedure OnAfterUndoPurchRcptLine(var PurchRcptLine: Record "Purch. Rcpt. Line")
    begin
    end;
}
